    ...
    switch(m_deathState)
    {
        ...
        case ALIVE:
        {
            ...
            if(GetCreatureInfo()->type == CREATURE_TYPE_MECHANICAL && GetZoneId() == 4197)
            {
                ...
                } else RemoveAurasDueToSpell(36444);
            }
...
void Creature::SetLootRecipient(Unit* unit)
{
    ...
    if(unit->GetTypeId() != TYPEID_PLAYER && !unit->IsVehicle()) //TODO: pets? unit->isPet()?
        return;
...
void Creature::setDeathState(DeathState s)
{
    ...
    if(s == JUST_DIED)
    {
        ...
        else // Set respawn time of creatures without loot to spawntimesecs value in creature table
        {
            m_corpseRemoveTime = time(NULL) + uint32(m_respawnDelay / 2);
...
uint32 Creature::GetVendorItemCurrentCount(VendorItem const* vItem)
{
    ...
    if(time_t(vCount->lastIncrementTime + vItem->incrtime) <= ptime)
    {
        ...
        if((vCount->count + diff) >= vItem->maxcount)
        {

...
void Unit::SendMonsterMove(MonsterMoveData const& moveData, Player* player)
{
    ...
    if(moveData.SplineFlag & SPLINEFLAG_UNKNOWN23)
    {
...
uint32 Unit::DealDamage(Unit* pVictim, uint32 damage, CleanDamage const* pCleanDMG, DamageEffectType damagetype, SpellSchoolMask damageSchoolMask, SpellEntry const* pSpell, bool durabilityLoss)
{
    ...
    if(pVictim && damage >= pVictim->GetHealth() && this->GetTypeId() == TYPEID_PLAYER)
    ...
    switch(spellProto->SpellFamilyName)
    {
        ...
        case SPELLFAMILY_DEATHKNIGHT:
            ...
            if(spellProto->SpellFamilyFlags[0] & 0x00002000) {
                AuraEffectList const& mDumyAuras = GetAuraEffectsByType(SPELL_AURA_DUMMY);
                for(AuraEffectList::const_iterator i = mDumyAuras.begin(); i != mDumyAuras.end(); ++i) {
                    if((*i)->GetSpellProto()->Id == 64962) {
                        AddPctN(DoneTotal, (*i)->GetBaseAmount());
                        break;
                    }
                }
            }
...
bool Unit::isSpellCrit(Unit* victim, SpellEntry const* spellProto, SpellSchoolMask schoolMask, WeaponAttackType attackType) const
{
    ...
    if(roll_chance_f(crit_chance) && !victim->HasAura(53480))
        return true;
    return false;
}
...
{
    if(!x)
        m_serverSideVisibility.SetValue(SERVERSIDE_VISIBILITY_GM, SEC_GAMEMASTER_LOW);
    else
...
void Unit::SetHover(bool on)    //TODO: see trinity core
{
    if(on)
...
int32 Unit::ModSpellDuration(SpellEntry const* spellProto, Unit const* target, int32 duration, bool positive, uint32 effectMask)
{
    ...
    if(!positive)
    {
        int32 mechanic = GetSpellMechanicMaskByEffectMask(spellProto, effectMask);
...
void Unit::ProcDamageAndSpellFor(bool isVictim, Unit* pTarget, uint32 procFlag, uint32 procExtra, WeaponAttackType attType, SpellEntry const* procSpell, uint32 damage, SpellEntry const* procAura)
{
    ...
    for(AuraApplicationMap::const_iterator itr = GetAppliedAuras().begin(); itr!= GetAppliedAuras().end(); ++itr)
    {
        ...
        if(procExtra & PROC_EX_ABSORB && isVictim)
            if(damage || spellProto->EffectTriggerSpell[0] || spellProto->EffectTriggerSpell[1] || spellProto->EffectTriggerSpell[2])
                active = true;
    ...
    for(ProcTriggeredList::const_iterator i = procTriggered.begin(); i != procTriggered.end(); ++i)
    {
        ...
        // if Damage is Absorbed do not take charges
        if(procExtra & (PROC_EX_ABSORB) && !damage && spellInfo->Id != 48111)
            takeCharges = false;
...
bool Unit::IsTriggeredAtSpellProcEvent(Unit* victim, Aura* aura, SpellEntry const* procSpell, uint32 procFlag, uint32 procExtra, WeaponAttackType attType, bool isVictim, bool active, SpellProcEventEntry const* & spellProcEvent)
{
    ...
    uint32 sfName = MAX_SPELLFAMILYNAME;
    if(aura && aura->GetOwner() && aura->GetOwner()->ToPlayer())
        sfName = SpellFamilyNameByClass[aura->GetOwner()->ToPlayer()->getClass()-1];
    spellProcEvent = sSpellMgr->GetSpellProcEvent(spellProto->Id, sfName);
...
void Unit::Kill(Unit* victim, bool durabilityLoss)
{
    ...
    if(victim->GetVehicle())
        victim->ExitVehicle();
    if(Vehicle* pVehicle = victim->GetVehicleKit())

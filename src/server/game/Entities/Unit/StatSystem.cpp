...
void Guardian::UpdateDamagePhysical(WeaponAttackType attType)
{
    ...
    if(int32 dmgPctMod = GetTotalAuraModifierByMiscMask(SPELL_AURA_MOD_DAMAGE_PERCENT_DONE, SPELL_SCHOOL_MASK_NORMAL))
    ...
    for(Unit::AuraEffectList::const_iterator itr = mDummy.begin(); itr != mDummy.end(); ++itr)
    {
        switch((*itr)->GetSpellProto()->Id)
        {
            case 61682:
            case 61683:
                AddPctN(mindamage, -(*itr)->GetAmount() / 2);
                AddPctN(maxdamage, -(*itr)->GetAmount() / 2);
                break;
            default:
                break;
        }

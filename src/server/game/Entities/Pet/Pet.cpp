...
void Creature::Regenerate(Powers power)
{
    ...
    switch(power)
    {
        case POWER_FOCUS:   // For hunter pets.
            addvalue = 24;
            break;
        case POWER_ENERGY:  // For deathknight's ghoul.
            addvalue = 20;
            break;
        default:
            return;
        ...
        default:
        {
            switch(GetEntry())
            {
                ...
                case 27829: // Ebon Gargoyle
                {
                    ...
                    if(Player *owner = m_owner->ToPlayer()) // get 100% of owning player's spellhaste
                    {
                        float bonus = owner->GetRatingBonusValue(CR_HASTE_SPELL);
                        bonus += owner->GetTotalAuraModifier(SPELL_AURA_MOD_CASTING_SPEED_NOT_STACK) + owner->GetTotalAuraModifier(SPELL_AURA_HASTE_SPELLS);
...
void Pet::_SaveAuras(SQLTransaction& trans)
{
    ...
    for(AuraMap::const_iterator itr = m_ownedAuras.begin(); itr != m_ownedAuras.end() ; ++itr)
    {
        ...
        trans->PAppend("INSERT IGNORE INTO `pet_aura` (guid, caster_guid, spell, effect_mask, recalculate_mask, stackcount, amount0, amount1, amount2, base_amount0, base_amount1, base_amount2, maxduration, remaintime, remaincharges) "

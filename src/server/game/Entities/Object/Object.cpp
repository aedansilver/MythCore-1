...
void Object::_BuildValuesUpdate(uint8 updatetype, ByteBuffer * data, UpdateMask *updateMask, Player* target) const
{
    ...
    {
        if(isType(TYPEMASK_GAMEOBJECT) && !ToGameObject()->IsDynTransport())
        {
            if(ToGameObject()->ActivateToQuest(target) || target->isGameMaster())
                IsActivateToQuest = true;

            updateMask->SetBit(GAMEOBJECT_DYNAMIC);

            if(ToGameObject()->GetGoArtKit())
                updateMask->SetBit(GAMEOBJECT_BYTES_1);
        }
        else if(isType(TYPEMASK_UNIT))
        {
            if(ToUnit()->HasFlag(UNIT_FIELD_AURASTATE, PER_CASTER_AURA_STATE_MASK))
            {
    ...
    {
        if(isType(TYPEMASK_GAMEOBJECT) && !ToGameObject()->IsTransport())
        {
            if(ToGameObject()->ActivateToQuest(target) || target->isGameMaster())
            {
                IsActivateToQuest = true;
            ...
            if (ToGameObject()->GetGoType() == GAMEOBJECT_TYPE_CHEST && ToGameObject()->GetGOInfo()->chest.groupLootRules && ToGameObject()->HasLootRecipient())
            ...
            if(updateMask->GetBit(index))
            {
                if(index == UNIT_NPC_FLAGS)
                {
                    ...
                    if(GetTypeId() == TYPEID_UNIT)
                    {
                        ...
                        {
                            if(!ToCreature()->isCanTrainingOf(target, false))
                ...
                else if(index == UNIT_FIELD_AURASTATE)
                {
                    ...
                    *data << uint32(ToUnit()->BuildAuraStateUpdateForTarget(target));
                }
                ...
                else if(index == UNIT_FIELD_DISPLAYID)
                {
                    if(GetTypeId() == TYPEID_UNIT)
                    {
                        const CreatureTemplate* cinfo = ToCreature()->GetCreatureInfo();
...
void WorldObject::SetMap(Map* pMap)
{
    ASSERT(pMap);
    ASSERT(!IsInWorld() || GetTypeId() == TYPEID_CORPSE);
    if(m_currMap == pMap || pMap == NULL) // Prevent add twice in one map or on map with id 0
        return;
    else if(m_currMap)
    {
        sLog->outCrash("WorldObject::SetMap: obj %u new map %u %u, old map %u %u", (uint32)GetTypeId(), pMap->GetId(), pMap->GetInstanceId(), m_currMap->GetId(), m_currMap->GetInstanceId());
        ASSERT(false);
    }
    m_currMap       = pMap;
    m_mapId         = pMap->GetId();
    m_InstanceId    = pMap->GetInstanceId();
    if(m_isWorldObject)

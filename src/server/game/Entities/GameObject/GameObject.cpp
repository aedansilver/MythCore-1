...
void GameObject::CleanupsBeforeDelete(bool /*finalCleanup*/)
{
    ...
    if(m_uint32Values)                                      // field array may not exist if GameOBject not loaded
        RemoveFromOwner();
}
...
void GameObject::Update(uint32 diff)
{
            ...
            if(isSpawned())
            {
                ...
                if(goInfo->type == GAMEOBJECT_TYPE_TRAP)
                {
                    ...
                    if(ok)
                    {
                        ...
                        if(owner && owner->isAlive() && owner->GetTypeId() == TYPEID_PLAYER)
    ...
    sScriptMgr->OnGameObjectUpdate(this, uint32(diff));
}
...
void GameObject::Use(Unit* user)
{
    ...
    switch(GetGoType())
    {
            ...
            if(GetUniqueUseCount() == info->summoningRitual.reqParticipants)
            {
                ...
                if (info->summoningRitual.casterTargetSpell && info->summoningRitual.casterTargetSpell != 1) // No idea why this field is a bool in some cases
                    for (uint32 i = 0; i < info->summoningRitual.casterTargetSpellTargets; i++)
                        // m_unique_users can contain only player GUIDs
                        if (Player* target = ObjectAccessor::GetPlayer(*this, Trinity::SelectRandomContainerElement(m_unique_users)))
        ...
        case GAMEOBJECT_TYPE_FLAGDROP:                      // 26
        {
            ...
            Player* pPlayer = user->ToPlayer();
            if(pPlayer->CanUseBattlegroundObject())
            {
                // in battleground check
                ...
                if(pPlayer->GetVehicle())
                    return;
                // BG flag dropped
     // War     // Warsong Gulch: 179785 - Alliance Flag
                // Warsong Gulch: 179786 - Horde Flag
                // Eye of the Storm: 184142
                GameObjectTemplate const* info = GetGOInfo();
                if(info)
                {
                    switch(info->entry)
                    {
                        case 179785:                        // Silverwing Flag
                            ...
                            if(pBG->IsRandom() ? pBG->GetTypeID(true) : pBG->GetTypeID(false) == BATTLEGROUND_WS)
                                pBG->EventPlayerClickedOnFlag(pPlayer, this);
                            break;
                        ...
                        case 184142:                        // Netherstorm Flag
                            if(pBG->IsRandom() ? pBG->GetTypeID(true) : pBG->GetTypeID(false) == BATTLEGROUND_EY)
                                pBG->EventPlayerClickedOnFlag(pPlayer, this);
                            break;
                    }
                }

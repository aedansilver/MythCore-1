...
ObjectGridRespawnMover::Visit(CreatureMapType &m)
{
    ...
    for(CreatureMapType::iterator iter = m.begin(); iter != m.end();)
    {
        ...
        {
            c->GetMap()->CreatureRespawnRelocation(c, true);    //TODO: CHECK THIS!!
...
void LoadHelper(CellCorpseSet const& cell_corpses, CellPair &cell, CorpseMapType &m, uint32 &count, Map* pMap)
{
    ...
    {
        if(itr->second != pMap->GetInstanceId())
            continue;

        uint32 player_guid = itr->first;

        Corpse* pCorpe = sObjectAccessor->GetCorpseForPlayerGUID(player_guid);
        if(!pCorpe)
            continue;
        ...
        // in that case pMap == currMap
        pCorpe->SetMap(pMap);

        AddObjectHelper(cell, m, count, pMap, pCorpe);
    }
}

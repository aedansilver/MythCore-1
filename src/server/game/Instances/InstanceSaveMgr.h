...
class InstanceSave
{
    ...
    public:
        ...
        void AddPlayer(Player* player) { TRINITY_GUARD(ACE_Thread_Mutex, m_lock); m_playerList.push_back(player); }
        bool RemovePlayer(Player* player) { TRINITY_GUARD(ACE_Thread_Mutex, m_lock); m_playerList.remove(player); return UnloadIfEmpty(); }

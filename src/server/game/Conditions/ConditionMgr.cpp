...
bool ConditionMgr::isSourceTypeValid(Condition* cond)
{
    ...
    switch(cond->mSourceType)
    {
        ...
        case CONDITION_SOURCE_TYPE_SPELL_SCRIPT_TARGET:
        {
            ...
            {
                sLog->outErrorDb("SourceEntry %u in `condition` table, has ConditionType %u. Only CONDITION_SPELL_SCRIPT_TARGET(18) is valid for CONDITION_SOURCE_TYPE_SPELL_SCRIPT_TARGET(13), ignoring.", cond->mSourceEntry, uint32(cond->mConditionType));
                return false;
            }

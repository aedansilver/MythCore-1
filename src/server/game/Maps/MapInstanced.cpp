...
*/
Map* MapInstanced::CreateInstance(const uint32 mapId, Player* pPlayer)
{
    ...
    if(IsBattlegroundOrArena())
    {
        ...
        pMap = _FindMap(NewInstanceId);
        if(!pMap)
            pMap = CreateBattleground(NewInstanceId, pPlayer->GetBattleground());
    }
    else
    {

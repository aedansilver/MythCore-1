...
void Channel::Join(uint64 p, const char *pass)
{
    ...
    if(plr)
    {
        if(HasFlag(CHANNEL_FLAG_LFG) &&
            sWorld->getBoolConfig(CONFIG_RESTRICTED_LFG_CHANNEL) && plr->GetSession()->GetSecurity() < SEC_GAMEMASTER_LOW && plr->GetGroup())
        {
            MakeNotInLfg(&data);
            SendToOne(&data, p);
            return;
    ...
    if(m_announce && (!plr || plr->GetSession()->GetSecurity() < SEC_GAMEMASTER_LOW || !sWorld->getBoolConfig(CONFIG_SILENTLY_GM_JOIN_TO_CHANNEL)))
    {
        MakeJoined(&data, p);
        SendToAll(&data);
    }
...
void Channel::KickOrBan(uint64 good, const char *badname, bool ban)
{
    ...
    else if(!players[good].IsModerator() && sec < SEC_GAMEMASTER_LOW)
    {
        WorldPacket data;
    ...
    else
    {
        ...
        else if(sec < SEC_GAMEMASTER_LOW && bad->GetGUID() == m_ownerGUID && good != m_ownerGUID)
        {
            WorldPacket data;
            MakeNotOwner(&data);
            SendToOne(&data, good);
...
void Channel::Password(uint64 p, const char *pass)
{
    ...
    else if(!players[p].IsModerator() && sec < SEC_GAMEMASTER_LOW)
    {
        WorldPacket data;
        MakeNotModerator(&data);
        SendToOne(&data, p);
...
void Channel::SetMode(uint64 p, const char *p2n, bool mod, bool set)
{
    ...
    else
    {
        ...
        if((plr->GetSession()->GetSecurity() < SEC_GAMEMASTER_LOW || newp->GetSession()->GetSecurity() < SEC_GAMEMASTER_LOW) &&

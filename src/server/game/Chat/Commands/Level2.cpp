...
bool ChatHandler::HandleMuteCommand(const char* args)
{
    ...
    if (GetSession() && GetSession()->GetSecurity() < SEC_GAMEMASTER_LOW)
        if (notspeaktime > 60)
            notspeaktime = 60;
    ...
    sWorld->SendTextByAccess(SEC_MODERATOR_NOANN, true, LANG_GMANNO_CHAT_DISABLED, m_session->GetPlayerName(), nameLink.c_str(), notspeaktime, mutereasonstr.c_str());
    sWorld->SendTextByAccess(SEC_MODERATOR_NOANN, false, LANG_ANNO_CHAT_DISABLED, nameLink.c_str(), notspeaktime, mutereasonstr.c_str());
...
bool ChatHandler::HandleUnmuteCommand(const char* args)
{
    ...
    sWorld->SendTextByAccess(SEC_MODERATOR_NOANN, true, LANG_GMANNO_CHAT_ENABLED, m_session->GetPlayerName(), nameLink.c_str());
    sWorld->SendTextByAccess(SEC_MODERATOR_NOANN, false, LANG_ANNO_CHAT_ENABLED, nameLink.c_str());

    if(target)
        ChatHandler(target).PSendSysMessage(LANG_YOUR_CHAT_ENABLED, m_session->GetPlayerName());

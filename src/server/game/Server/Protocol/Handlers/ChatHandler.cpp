...
void WorldSession::HandleMessagechatOpcode(WorldPacket & recv_data)
{
    ...
    if(langDesc->skill_id != 0 && !pSender->HasSkill(langDesc->skill_id))
    {
        // also check SPELL_AURA_COMPREHEND_LANGUAGE (client offers option to speak in that language)
        Unit::AuraEffectList const& langAuras = pSender->GetAuraEffectsByType(SPELL_AURA_COMPREHEND_LANGUAGE);
    ...
    if(lang == LANG_ADDON)
    {
        if(sWorld->getBoolConfig(CONFIG_CHATLOG_ADDON))
        {
            ...
            sScriptMgr->OnPlayerChat(pSender, uint32(CHAT_MSG_ADDON), lang, msg);
        }
    ...
    else
    {
        ...
        else
        {
            ...
            Unit::AuraEffectList const& ModLangAuras = pSender->GetAuraEffectsByType(SPELL_AURA_MOD_LANGUAGE);
        ...
        if(type != CHAT_MSG_AFK && type != CHAT_MSG_DND)
            pSender->UpdateSpeakTime();
    }

    if(pSender->HasAura(1852) && type != CHAT_MSG_WHISPER)
    {
        std::string msg="";
        recv_data >> msg;

        SendNotification(GetTrinityString(LANG_GM_SILENCE), pSender->GetName());
        return;
    }
    ...
    switch(type)
    {
        ...
        case CHAT_MSG_YELL:
        {
            if(pSender->getLevel() < sWorld->getIntConfig(CONFIG_CHAT_SAY_LEVEL_REQ))
            {
            ...
            if(type == CHAT_MSG_SAY)
                pSender->Say(msg, lang);
            else if(type == CHAT_MSG_EMOTE)
                pSender->TextEmote(msg);
            else if(type == CHAT_MSG_YELL)
                pSender->Yell(msg, lang);
        } break;
        case CHAT_MSG_WHISPER:
        {
            if(pSender->getLevel() < sWorld->getIntConfig(CONFIG_CHAT_WHISPER_LEVEL_REQ))
            {
            ...
            Player* pReceiver = sObjectMgr->GetPlayer(to.c_str());

            uint32 senderSecurity = GetSecurity();
            uint32 receiverSecurity = pReceiver ? pReceiver->GetSession()->GetSecurity() : SEC_PLAYER;
            if(!pReceiver || (senderSecurity == SEC_PLAYER && receiverSecurity > SEC_PLAYER && !pReceiver->isAcceptWhispers() && !pReceiver->IsInWhipserWhiteList(pSender->GetGUID())))
            {
                SendPlayerNotFoundNotice(to);
                return;
            }
            ...
            if(!sWorld->getBoolConfig(CONFIG_ALLOW_TWO_SIDE_INTERACTION_CHAT) && senderSecurity == SEC_PLAYER && receiverSecurity == SEC_PLAYER)
            {
                ...
                uint32 receiverFaction = pReceiver->GetTeam();
            ...
            if(GetPlayer()->HasAura(1852) && !pReceiver->isGameMaster())
            {
            ...
            if(pSender->GetSession()->GetSecurity() > SEC_PLAYER && !pSender->isAcceptWhispers() && !pSender->IsInWhipserWhiteList(pReceiver->GetGUID()))
                pSender->AddWhisperWhiteList(pReceiver->GetGUID());

            GetPlayer()->Whisper(msg, lang, pReceiver->GetGUID());;
        } break;
        case CHAT_MSG_PARTY:
        case CHAT_MSG_PARTY_LEADER:
        {
...
void WorldSession::HandleChatIgnoredOpcode(WorldPacket& recv_data)
{
    ...
    Player* pPlayer = sObjectMgr->GetPlayer(iguid);

...
void WorldSession::HandleStableSwapPetCallback(QueryResult result, uint32 petnumber)
{
    ...
    Pet* _pPet = _player->GetPet();
    if(!_pPet || _pPet->getPetType() != HUNTER_PET)
        return;

    _player->RemovePet(_pPet, _pPet->isAlive() ? PetSaveMode(slot) : PET_SAVE_AS_DELETED);

    // summon unstabled pet
    Pet* pPet = new Pet(_player);
    if(!pPet->LoadPetFromDB(_player, creature_id, petnumber))
    {
        delete pPet;
        SendStableResult(STABLE_ERR_STABLE);
    } else SendStableResult(STABLE_SUCCESS_UNSTABLE);
}

...
bool ObjectMgr::LoadTrinityStrings(char const* table, int32 min_value, int32 max_value)
{
    ...
    sLog->outString(">> Loaded %u Myth strings in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
...
bool LoadTrinityStrings(char const* table, int32 start_value, int32 end_value)
{
    ...
    {
        sLog->outErrorDb("Table '%s' load attempted with range (%d - %d) reserved by Myth, strings not loaded.", table, start_value, end_value+1);
        return false;
    }

...
FleeingMovementGenerator<T>::_setTargetLocation(T &owner)
{
    ...
    // Disable Flee to pass into these objects (Arena)
    if(owner.FindNearestGameObject(401006, 1.0f) ||
        owner.FindNearestGameObject(194581, 1.0f) ||
        owner.FindNearestGameObject(194580, 1.0f) ||
        owner.FindNearestGameObject(194579, 1.0f) ||
        owner.FindNearestGameObject(194578, 1.0f) ||
        owner.FindNearestGameObject(194395, 1.0f) ||
        owner.FindNearestGameObject(191877, 1.0f))
        return;
...
FleeingMovementGenerator<T>::_getPoint(T &owner, float &x, float &y, float &z)
{
    ...
    for(uint8 i = 0; i < 18; ++i)
    {
        ...
        if(owner.IsWithinLOS(temp_x, temp_y, z))
        {
            ...
            {
                float new_z_left = _map->GetHeight(owner.GetPhaseMask(), temp_x + (float)(cos(angle+M_PI/2)), temp_y + (float)(sin(angle+M_PI/2)), z, true);
                float new_z_right = _map->GetHeight(owner.GetPhaseMask(), temp_x + (float)(cos(angle-M_PI/2)), temp_y + (float)(sin(angle-M_PI/2)), z, true);

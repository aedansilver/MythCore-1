...
{
    if(!i_owner || i_owner->HasUnitState(UNIT_STAT_ROOT | UNIT_STAT_STUNNED)) // what about UNIT_STAT_DISTRACTED? Why is this not included?
        return;
...
void MotionMaster::MoveLand(uint32 id, Position const& pos, float speed)
{
    ...
    uint32 moveFlag = SPLINEFLAG_FLYING | SPLINEFLAG_UNKNOWN23;

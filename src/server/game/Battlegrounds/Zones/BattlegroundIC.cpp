...
void BattlegroundIC::AddPlayer(Player* pPlayer)
{
    Battleground::AddPlayer(pPlayer);
    pPlayer->Achievement3853();
    //create score and add it to map, default values are set in constructor
    BattlegroundICScore* sc = new BattlegroundICScore;

    m_PlayerScores[pPlayer->GetGUID()] = sc;

    if(nodePoint[NODE_TYPE_QUARRY].nodeState == (pPlayer->GetTeamId() == TEAM_ALLIANCE ? NODE_STATE_CONTROLLED_A : NODE_STATE_CONTROLLED_H))
        pPlayer->CastSpell(pPlayer, SPELL_QUARRY, true);

    if(nodePoint[NODE_TYPE_REFINERY].nodeState == (pPlayer->GetTeamId() == TEAM_ALLIANCE ? NODE_STATE_CONTROLLED_A : NODE_STATE_CONTROLLED_H))
        pPlayer->CastSpell(pPlayer, SPELL_OIL_REFINERY, true);

    SendTransportInit(pPlayer);
}

void BattlegroundIC::RemovePlayer(Player* pPlayer, uint64 /*guid*/, uint32 /*team*/)
{
    if(!pPlayer)
        return;

    pPlayer->RemoveAura(SPELL_QUARRY);
    pPlayer->RemoveAura(SPELL_OIL_REFINERY);
    pPlayer->Achievement3853();
}

void BattlegroundIC::HandleAreaTrigger(Player* /*pSource*/, uint32 /*Trigger*/)
{
...
void BattlegroundIC::HandleKillPlayer(Player* pVictim, Player* pKiller)
{
    if(GetStatus() != STATUS_IN_PROGRESS)
        return;
    ...
    factionReinforcements[pVictim->GetTeamId()] -= 1;

    UpdateWorldState((pVictim->GetTeamId() == TEAM_ALLIANCE ? BG_IC_ALLIANCE_RENFORT : BG_IC_HORDE_RENFORT), factionReinforcements[pVictim->GetTeamId()]);

    switch(pKiller->GetZoneId())
    {
        case 4747:
            if(pKiller->getFaction() == HORDE && pVictim->getFaction() == ALLIANCE)
                pKiller->T4747 = true;
            else if(pKiller->getFaction() == ALLIANCE && pVictim->getFaction() == HORDE)
                pKiller->T4747 = true;
            break;
        case 4749:
            if(pKiller->getFaction() == HORDE && pVictim->getFaction() == ALLIANCE)
    ...
    pKiller->Achievement3853Check();

    // If no reinforcents -> END!
    if(factionReinforcements[pVictim->GetTeamId()] < 1)
        EndBattleground(pKiller->GetTeam());
}
...
void BattlegroundIC::EventPlayerDamagedGO(Player* /*pPlayer*/, GameObject* /*pGo*/, uint32 /*eventType*/) { }

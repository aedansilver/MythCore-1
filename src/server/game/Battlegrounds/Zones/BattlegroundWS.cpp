...
void BattlegroundWS::Update(uint32 diff)
{
    ...
    if(GetStatus() == STATUS_IN_PROGRESS)
    {
        ...
        if(m_BothFlagsKept)
        {
            ...
            {
                if(Player* pPlayer = sObjectMgr->GetPlayer(m_FlagKeepers[0]))
                    pPlayer->CastSpell(pPlayer, WS_SPELL_FOCUSED_ASSAULT, true);
                if(Player* pPlayer = sObjectMgr->GetPlayer(m_FlagKeepers[1]))
                    pPlayer->CastSpell(pPlayer, WS_SPELL_FOCUSED_ASSAULT, true);
                m_FlagDebuffState = 1;
            } else if(m_FlagDebuffState == 1 && m_FlagSpellForceTimer >= 900000) { //15 minutes
                if(Player* pPlayer = sObjectMgr->GetPlayer(m_FlagKeepers[0]))
                {
                    pPlayer->RemoveAurasDueToSpell(WS_SPELL_FOCUSED_ASSAULT);
                    pPlayer->CastSpell(pPlayer, WS_SPELL_BRUTAL_ASSAULT, true);
                }
...
void BattlegroundWS::HandleKillPlayer(Player* pVictim, Player* pKiller)
{
    if(GetStatus() != STATUS_IN_PROGRESS)
        return;
    if(pVictim->HasAura(BG_WS_SPELL_SILVERWING_FLAG) && pKiller->getFaction() == HORDE)
        pKiller->UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_SPECIAL_PVP_KILL); //Criteria: 3699
    if(pVictim->HasAura(BG_WS_SPELL_WARSONG_FLAG) && pKiller->getFaction() == ALLIANCE)
        pKiller->UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_SPECIAL_PVP_KILL); //Criteria: 3698

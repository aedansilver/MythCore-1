...
void BattlegroundSA::HandleAreaTrigger(Player* /*pSource*/, uint32 /*Trigger*/)
{
...
void BattlegroundSA::EventPlayerDamagedGO(Player* /*pPlayer*/, GameObject* go, uint32 eventType)
{
    if(!go || !go->GetGOInfo())
        return;
    ...
    {
        uint32 i = GetGateIDFromDamagedEventID(eventType);
...
void BattlegroundSA::DemolisherStartState(bool start)
{
    ...
    for(uint8 i = BG_SA_DEMOLISHER_1; i <= BG_SA_DEMOLISHER_4; i++)
    {
        if(Creature* pVehicle = GetBGCreature(i))
        {
            if(start)
                pVehicle->ForcedDespawn();
            else
                pVehicle->Respawn(true);
        }
    }
}

...
void BattlegroundAV::HandleKillPlayer(Player* pVictim, Player* pKiller)
{
    ...
    UpdateScore(pVictim->GetTeam(), -1);
    pKiller->T425 = true;
    pKiller->Achievement582Check();
}
...
void BattlegroundAV::EventPlayerDefendsPoint(Player* pPlayer, uint32 object)
{
    ...
    if(owner == pPlayer->GetTeam() || m_Nodes[node].State != POINT_ASSAULTED)
        return;
    ...
    { //until snowfall doesn't belong to anyone it is better handled in assault-code
        ...
        EventPlayerAssaultsPoint(pPlayer, object);
        return;
    }
    sLog->outDebug(LOG_FILTER_BATTLEGROUND, "pPlayer defends point object: %i node: %i", object, node);
    if(m_Nodes[node].PrevOwner != team)
    {
        sLog->outError("BG_AV: pPlayer defends point which doesn't belong to his team %i", node);
        return;
    }
   ...
   //spawn new go :)
    ...
    UpdatePlayerScore(pPlayer, (IsTower(node)) ? SCORE_TOWERS_DEFENDED : SCORE_GRAVEYARDS_DEFENDED, 1);
    if(IsTower(node))
    ...
    pPlayer->T422 = true;
    pPlayer->T424 = true;
    pPlayer->Achievement582Check();
}

void BattlegroundAV::EventPlayerAssaultsPoint(Player* pPlayer, uint32 object)
{
    ASSERT(GetStatus() == STATUS_IN_PROGRESS);
    ...
    uint32 team  = pPlayer->GetTeam();
    sLog->outDebug(LOG_FILTER_BATTLEGROUND, "bg_av: pPlayer assaults point object %i node %i", object, node);
    ...
    UpdatePlayerScore(pPlayer, (IsTower(node)) ? SCORE_TOWERS_ASSAULTED : SCORE_GRAVEYARDS_ASSAULTED, 1);

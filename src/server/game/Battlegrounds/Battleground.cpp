    ...
    if(isArena() && isRated())
    {
        ...
        if(winner_arena_team && loser_arena_team && winner_arena_team != loser_arena_team)
        {
            if(winner != WINNER_NONE)
            {
                ...
                if(loser_change < 1 && winner_change < abs(loser_change)) {
                    winner_change = abs(loser_change) + 1;
                    sLog->outArena("   > Modified: Winner change = %d.", winner_change);
                }
        ...
        if(isArena() && isRated() && winner_arena_team && loser_arena_team && winner_arena_team != loser_arena_team)
        {
            if(team == winner)
            {
                ...
                if(member)
                {
                    ...
                    plr->GetAchievementMgr().UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_WIN_ARENA, 1, GetArenaType());
                }
            ...
            plr->GetAchievementMgr().UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_PLAY_ARENA, 1, GetArenaType());
        }
...
bool Battleground::AddObject(uint32 type, uint32 entry, float x, float y, float z, float o, float rotation0, float rotation1, float rotation2, float rotation3, uint32 /*respawnTime*/)
{
    ...
    if(!pGo->Create(sObjectMgr->GenerateLowGuid(HIGHGUID_GAMEOBJECT), entry, GetBgMap(),

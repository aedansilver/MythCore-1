...
inline uint32 GetSpellMechanicMaskByEffectMask(SpellEntry const* spellProto, uint32 effectMask)
{
    uint32 mask = 0;
    if(spellProto->Mechanic)
        mask |= 1<< spellProto->Mechanic;
    for(int i = 0; i < MAX_SPELL_EFFECTS; ++i)
        if((effectMask & (1 << i)) && spellProto->EffectMechanic[i])
            mask |= 1<< spellProto->EffectMechanic[i];
    return mask;
}
...
typedef std::multimap<uint32, SpellProcEventEntry> SpellProcEventMap;
#define MAX_SPELLFAMILYNAME 18
...
class SpellMgr
{
    ...
    public:
        ...
        // Spell proc events
        SpellProcEventEntry const* GetSpellProcEvent(uint32 spellId, uint32 spellFamilyName = MAX_SPELLFAMILYNAME) const
        {
            switch(mSpellProcEventMap.count(spellId))
            {
            default:
                {
                if(spellFamilyName == MAX_SPELLFAMILYNAME)
                {
                    sLog->outErrorDb("GetSpellProcEvent: several entries in spell_proc_event for spell %u, but no preferred! Getting the first one.", spellId);
                    break;
                }
                std::pair<SpellProcEventMap::const_iterator,SpellProcEventMap::const_iterator> range = mSpellProcEventMap.equal_range(spellId);
                for(SpellProcEventMap::const_iterator it = range.first; it != range.second; ++it)
                    if(it->second.spellFamilyName == spellFamilyName)
                        return &it->second;
                }
            //no break intended
            case 0:

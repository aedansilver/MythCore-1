        ...
        switch(m_spellInfo->SpellFamilyName)
        {
            case SPELLFAMILY_GENERIC:
            {
                ...
                switch(m_spellInfo->Id)                     // better way to check unknown
                {
                    ...
                    {
                        float distance = m_caster->GetDistance2d(unitTarget);
                        damage *= (int32)exp(-distance/15.0f);
                        break;
                    }
                }
                break;
            ...
            case SPELLFAMILY_ROGUE:
            {
                ...
                {
                    if(Unit* pCreator = m_caster->GetCharmerOrOwner())
                    {
                        if(Aura* pAura = pCreator->GetAura(62218))
                        {
                            float ownAP  = pCreator->GetTotalAttackPowerValue(BASE_ATTACK);
                            int   base   = SpellMgr::CalculateSpellEffectAmount(m_spellInfo, EFFECT_0);
                            float stacks = float(pAura->GetStackAmount());
                            int sharedmg = int32(ownAP * base * 0.01f);

                            int mindmg = int32(sharedmg + ownAP * (stacks / 20));
                            int maxdmg = int32(sharedmg + ownAP * (stacks / 10));

                            damage = urand(mindmg, maxdmg);
                            pCreator->RemoveAurasDueToSpell(62218);
                        } else return;
                    }
                }
                break;
            ...
            case SPELLFAMILY_DEATHKNIGHT:
            {
                ...
                else if(m_spellInfo->SpellFamilyFlags[0] & 0x00040000) // Blood Boil - bonus for diseased targets
                    {
...
void Spell::EffectDummy(SpellEffIndex effIndex)
{
    ...
    switch(m_spellInfo->SpellFamilyName)
    {
        case SPELLFAMILY_GENERIC:
        {
            switch(m_spellInfo->Id)
            {
                case 74035: //Motivate
                {
                     ...
                     if((unitTarget->GetEntry() == 39623) && (!pCreature->HasUnitTypeMask(UNIT_MASK_MINION)))
                     {
                         pCreature->AddUnitTypeMask(UNIT_MASK_MINION);
                         pCreature->AddUnitTypeMask(UNIT_STAT_FOLLOW);
                         pCreature->GetMotionMaster()->MoveFollow(m_caster, PET_FOLLOW_DIST, PET_FOLLOW_ANGLE);
                         pCreature->SetSpeed(MOVE_RUN, 1.70f);
                ...
                case 38782: //Druid Signal
                {
                    ...
                    Creature* creature = m_caster->FindNearestCreature(22423, 20, true);
                    if(!creature)
                    {
                        Creature* summon = m_caster->SummonCreature(22423, 2034.37f, 5426.76f, 160.172f, 2.69017f,  TEMPSUMMON_DEAD_DESPAWN, 0);
                        summon->RemoveFlag(UNIT_NPC_FLAGS, UNIT_NPC_FLAG_GOSSIP);
                        summon->SetDisplayId(18380);
                    }
                     return;
                }
                ...
                case 54171:                                   //Divine Storm
                {
                    ...
                    {
                        float GlyphYesOrNot = 0.25f;
                        if(m_caster->HasAura(63220))
                            GlyphYesOrNot = 0.4f;

                        int32 heal = int32(damage * GlyphYesOrNot);
        ...
        case SPELLFAMILY_DRUID:
            ...
            } else if(m_spellInfo->Id == 5229 && m_caster->HasAura(70726))
        ...
        case SPELLFAMILY_DEATHKNIGHT:
            ...
            switch(m_spellInfo->Id)
            {
                case 49560: // Death Grip
                    if(m_caster->isJumping() || m_caster->IsFlying())
                        return;
                ...
                case 46584: // Raise Dead
                    ...
                    if(m_caster->HasAura(52143))
                        bp = 52150;     // summon as pet
                    else bp = 46585;    // or guardian
                    if(m_targets.HasDst())
    ...
    if(spell_id)
    {
        ...
        Spell* pSpell = new Spell(m_caster, spellInfo, triggered, m_originalCasterGUID, true);
        if(bp) pSpell->SetSpellValue(SPELLVALUE_BASE_POINT0, bp);
...
void Spell::EffectTeleportUnits(SpellEffIndex /*effIndex*/)
{
    ...
    switch(m_spellInfo->Id)
    {
        ...
        case 60320:  // Scroll of Recall II
            uiMaxSafeLevel = uiMaxSafeLevel ? uiMaxSafeLevel : 70;
        case 60321:  // Scroll of Recall III
            uiMaxSafeLevel = uiMaxSafeLevel ? uiMaxSafeLevel : 80;
    ...
    switch(m_spellInfo->Id)
    {
        ...
        case 36890:
        {
            if(roll_chance_i(50))                        // 50% success
            {
                ...
                switch(rand_eff)
                {
                    case 1: // soul split - evil
                        m_caster->CastSpell(m_caster, 36900, true);
                        break;
                    case 2: // soul split - good
                        m_caster->CastSpell(m_caster, 36901, true);
                        break;
                    case 3: // Increase the size
                        m_caster->CastSpell(m_caster, 36895, true);
                        break;
                    case 4: // Transform
                    {
...
void Spell::EffectApplyAura(SpellEffIndex effIndex)
{
    ...
    switch(m_spellInfo->Id)
    {
        ...
        case 62853: // Lance Equipped
        {
            if(m_caster)
            {
                if(Player* pPlayer = m_caster->ToPlayer())
                {
                    if(pPlayer->GetAreaId() == 4670)
...
void Spell::EffectDistract(SpellEffIndex /*effIndex*/)
{
    ...
    } else {    // Set creature Distracted, Stop it, And turn it
    ...
    switch(m_spellInfo->SpellFamilyName)
    {
        case SPELLFAMILY_GENERIC:
        {
            switch(m_spellInfo->Id)
            {
                ...
                //Teleport to Lake Wintergrasp TODO: move to spellscript! and discard #include OutdoorPvPWG
                case 58622:
                {
                ...
                case 66480: // Shield - Breaker, 10k Damage
                case 64595: // Shield - Breaker, 10k Damage
            ...
            switch(m_spellInfo->Id)
            {
                ...
                case 53271:
                {
                    ...
                    if(Pet* pPet = m_caster->ToPlayer()->GetPet())
                        m_caster->CastSpell(pPet, 62305, true);
                        return;
                }
                default:
                    break;
        ...
        case SPELLFAMILY_PALADIN:
        {
            ...
            if(m_spellInfo->Category == SPELLCATEGORY_JUDGEMENT)
            {
                ...
                if(m_caster->HasAura(20335))
                    m_caster->CastSpell(unitTarget, 21183, true);
                else if(m_caster->HasAura(20336))
                    m_caster->CastSpell(unitTarget, 54498, true);
                else if(m_caster->HasAura(20337))
                    m_caster->CastSpell(unitTarget, 54499, true);
                return;
            }
        }
...
void Spell::EffectSummonPlayer(SpellEffIndex /*effIndex*/)
{
    ...
    unitTarget->ToPlayer()->SetSummonPoint(m_caster->GetMapId(), m_caster->GetPositionX(), m_caster->GetPositionY(), m_caster->GetPositionZ());
...
{
    if(!unitTarget || !unitTarget->IsInWorld() || unitTarget->isAlive() || unitTarget->GetTypeId() != TYPEID_PLAYER)
        return;
...
void Spell::EffectActivateRune(SpellEffIndex effIndex)
{
    ...
    if(!pPlayer || pPlayer->getClass() != CLASS_DEATH_KNIGHT)
        return;
    ...
    {
        if(pPlayer->GetRuneCooldown(j) && pPlayer->GetCurrentRune(j) == RuneType(m_spellInfo->EffectMiscValue[effIndex]))
        {
            pPlayer->SetRuneCooldown(j, 0);
            --count;
        }
    }

    if(m_spellInfo->Id == 45529 && count > 0)    // Blood Tap
    {
        for(uint32 j = 0; j < MAX_RUNES && count > 0; ++j)
        {
            if(pPlayer->GetRuneCooldown(j) && pPlayer->GetCurrentRune(j) == RUNE_BLOOD)
            {
                pPlayer->SetRuneCooldown(j, 0);
                --count;
            }
    ...
    if(m_spellInfo->Id == 47568)
    {
        ...
        {
            if(pPlayer->GetRuneCooldown(i) && (pPlayer->GetCurrentRune(i) == RUNE_FROST ||  pPlayer->GetCurrentRune(i) == RUNE_DEATH))
                pPlayer->SetRuneCooldown(i, 0);
        }
    }

    pPlayer->ResyncRunes(MAX_RUNES);
}
...
void Spell::EffectCreateTamedPet(SpellEffIndex effIndex)
{
    ...
    Pet* pPet = unitTarget->CreateTamedPetFrom(creatureEntry, m_spellInfo->Id);
    if(!pPet)
        return;

    // add to world
    pPet->GetMap()->Add(pPet->ToCreature());
    // unitTarget has pPet now
    unitTarget->SetMinion(pPet, true);
...
void Spell::EffectCastButtons(SpellEffIndex effIndex)
{
    ...
    for(; n_buttons; n_buttons--, button_id++)
    {
        ...
        // hack for Glyph of Totem of Wrath. If you merge your branch to last revisions trinity - not add this code.
        if(m_caster->HasAura(63280) && spellInfo->SpellIconID ==  2019)
        {
            int32 triggerAmount = 30; //Glyph add 30 percent from totem effect
            AuraEffect* aurEffA = NULL;
            std::list<AuraEffect *> const& auras = m_caster->GetAuraEffectsByType(SPELL_AURA_MOD_DAMAGE_DONE);
            for(std::list<AuraEffect *>::const_iterator i = auras.begin(); i != auras.end(); ++i)
            {
            ...
            if(aurEffA)
            {
                ...
                m_caster->CastCustomSpell(unitTarget, 63283, &bp0, &bp1, NULL, true); //, triggeredByAura);
            }
        }

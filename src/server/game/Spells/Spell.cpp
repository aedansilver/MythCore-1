...
void Spell::DoAllEffectOnTarget(TargetInfo *target)
{
    ...
    else if(missInfo == SPELL_MISS_REFLECT)                // In case spell reflect from target, do all effect on caster (if hit)
    {
        if(target->reflectResult == SPELL_MISS_NONE)       // If reflected spell hit caster -> do all effect on him
        {
            ...
            DoSpellHitOnUnit(m_caster, mask, target->scaleAura);
        }
    }
                    ...
                    if(durationadd)
                    {
                        switch(m_diminishLevel)
                        {
                            ...
                            case DIMINISHING_LEVEL_2:
                                duration += 1000; mod = 0.5f; break;
                            case DIMINISHING_LEVEL_3:
                                duration += 1000; mod = 0.25f; break;
...
bool Spell::UpdateChanneledTargetList()
{
    ...
    for(std::list<TargetInfo>::iterator ihit= m_UniqueTargetInfo.begin(); ihit != m_UniqueTargetInfo.end(); ++ihit)
    {
        ...
        {
            Unit* pUnit = m_caster->GetGUID() == ihit->targetGUID ? m_caster : ObjectAccessor::GetUnit(*m_caster, ihit->targetGUID);

            if(!pUnit)
                continue;

            if(IsValidDeadOrAliveTarget(pUnit))
            {
                if(channelAuraMask & ihit->effectMask)
                {
                    if(AuraApplication * aurApp = pUnit->GetAuraApplication(m_spellInfo->Id, m_originalCasterGUID))
                    {
                        if(m_caster != pUnit && !m_caster->IsWithinDistInMap(pUnit, range))
                        {
                            ihit->effectMask &= ~aurApp->GetEffectMask();
                            pUnit->RemoveAura(aurApp);
                            continue;
                        }
...
void Spell::SendSpellGo()
{
    ...
    if(castFlags & CAST_FLAG_RUNE_LIST)                   // rune cooldowns list
    {
        ...
        uint8 runeMaskAfterCast = pPlayer->GetRunesState();
        ...
        for(uint8 i = 0; i < MAX_RUNES; ++i)
        {
            ...
            if(mask & runeMaskInitial && !(mask & runeMaskAfterCast))  // usable before andon cooldown now...
            {
                ...
                float baseCd = float(pPlayer->GetRuneBaseCooldown(i));
                data << uint8((baseCd - float(pPlayer->GetRuneCooldown(i))) / baseCd * 255); // rune cooldown passed
            }
        }
    }
...
void Spell::TakeRunePower(bool didHit)
{
    ...
    SpellRuneCostEntry const* pRune = sSpellRuneCostStore.LookupEntry(m_spellInfo->runeCostID);

    if(!pRune || (pRune->NoRuneCost() && pRune->NoRunicPowerGain()))
        return;

    m_runesState = pPlayer->GetRunesState();                // store previous state

    int runeCost[NUM_RUNE_TYPES];                           // blood, frost, unholy, death

    for(int i = 0; i < RUNE_DEATH; ++i)
    {
        runeCost[i] = pRune->RuneCost[i];
        if(Player* modOwner = m_caster->GetSpellModOwner())
            modOwner->ApplySpellMod(m_spellInfo->Id, SPELLMOD_COST, runeCost[i], this);
    }
    ...
    {
        RuneType rune = pPlayer->GetCurrentRune(i);
        if(!pPlayer->GetRuneCooldown(i) && runeCost[rune] > 0)
        {
            pPlayer->SetRuneCooldown(i, didHit ? pPlayer->GetRuneBaseCooldown(i) : uint32(RUNE_MISS_COOLDOWN));
            pPlayer->SetLastUsedRune(rune);
            runeCost[rune]--;
        }
    }
    ...
    if(runeCost[RUNE_DEATH] > 0)
    {
        ...
        {
            RuneType rune = pPlayer->GetCurrentRune(i);
            if(!pPlayer->GetRuneCooldown(i) && rune == RUNE_DEATH)
            {
                pPlayer->SetRuneCooldown(i, didHit ? pPlayer->GetRuneBaseCooldown(i) : (uint32)RUNE_MISS_COOLDOWN);
                pPlayer->SetLastUsedRune(rune);
                runeCost[rune]--;
    ...
    if(didHit)
        if(pRune->runePowerGain)
            pPlayer->ModifyPower(POWER_RUNIC_POWER, int(pRune->runePowerGain));
}
...
SpellCastResult Spell::CheckCast(bool strict)
{
    ...
    if(target)
    {
        ...
        if(non_caster_target)
        {
            ...
            if(target->HasUnitState(UNIT_STAT_IN_FLIGHT) /*|| (target->HasUnitState(UNIT_STAT_ONVEHICLE) && target->GetVehicleBase() != m_caster)*/) // SUN CHECK!!
                return SPELL_FAILED_BAD_TARGETS;
    ...
    if(!m_IsTriggeredSpell && (m_spellInfo->Effect[EFFECT_0] == SPELL_EFFECT_DISPEL))
        if(Unit const * target = m_targets.GetUnitTarget())
            ...
            {
                bool check = true;
                dispelMask = GetDispelMask(DispelType(m_spellInfo->EffectMiscValue[EFFECT_0]));
            ...
            case SPELL_AURA_MOD_INCREASE_MOUNTED_FLIGHT_SPEED:
            {
                ...
                if(m_originalCaster && m_originalCaster->GetTypeId() == TYPEID_PLAYER && m_originalCaster->isAlive())
                {
                    if(AreaTableEntry const* pArea = GetAreaEntryByAreaID(m_originalCaster->GetAreaId()))
                    {
                        ...
                        {
                            if(m_originalCaster->GetZoneId() == 4197 && pvpWG && pvpWG->isWarTime())
                                  return m_IsTriggeredSpell ? SPELL_FAILED_DONT_REPORT : SPELL_FAILED_NOT_HERE;
                        }
                    }
                }
...
SpellCastResult Spell::CheckItems()
{
    ...
    else if(m_caster->GetTypeId() == TYPEID_PLAYER && !m_caster->ToPlayer()->HasItemFitToSpellRequirements(m_spellInfo))
        return SPELL_FAILED_EQUIPPED_ITEM_CLASS;
...
bool Spell::CheckTargetCreatureType(Unit* pTarget) const
{
    uint32 spellCreatureTargetMask = m_spellInfo->TargetCreatureType;

    // Curse of Doom & Exorcism: not find another way to fix spell pTarget check :/
    ...
    if(pTarget->GetEntry() == 5925 && m_spellInfo->SpellFamilyName == SPELLFAMILY_MAGE && (m_spellInfo->SpellFamilyFlags[0] & 0x1000000) && m_spellInfo->EffectApplyAuraName[0] == SPELL_AURA_MOD_CONFUSE)
        return true;
...
bool Spell::CheckTarget(Unit* pTarget, uint32 eff)
{
    // Check targets for creature type mask and remove not appropriate (skip explicit self pTarget case, maybe need other explicit targets)
    if(m_spellInfo->EffectImplicitTargetA[eff] != TARGET_UNIT_CASTER)
    {
        if(!CheckTargetCreatureType(pTarget))
            return false;
    }

    // Check Aura spell req (need for AoE spells)
    if(m_spellInfo->targetAuraSpell && !pTarget->HasAura(sSpellMgr->GetSpellIdForDifficulty(m_spellInfo->targetAuraSpell, m_caster)))
        return false;
    if(m_spellInfo->excludeTargetAuraSpell && pTarget->HasAura(sSpellMgr->GetSpellIdForDifficulty(m_spellInfo->excludeTargetAuraSpell, m_caster)))
        return false;

    // Check targets for not_selectable unit flag and remove
    // A player can cast spells on his pet (or other controlled unit) though in any state
    if(pTarget != m_caster && pTarget->GetCharmerOrOwnerGUID() != m_caster->GetGUID())
    {
        // any unattackable pTarget skipped
        if(pTarget->HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NON_ATTACKABLE))
            return false;
    }

    //Check player targets and remove if in GM mode or GM invisibility (for not self casting case)
    if(pTarget != m_caster && pTarget->GetTypeId() == TYPEID_PLAYER)
    {
        if(!pTarget->ToPlayer()->IsVisible())
            return false;

        if(pTarget->ToPlayer()->isGameMaster() && !IsPositiveSpell(m_spellInfo->Id))
            return false;
    }
    ...
    switch(m_spellInfo->EffectApplyAuraName[eff])
    {
        ...
        case SPELL_AURA_MOD_POSSESS_PET:
        case SPELL_AURA_AOE_CHARM:
            if(pTarget->GetTypeId() == TYPEID_UNIT && pTarget->IsVehicle())
                return false;
            if(pTarget->IsMounted())
                return false;
            if(pTarget->GetCharmerGUID())
                return false;
            if(int32 damage = CalculateDamage(eff, pTarget))
                if((int32)pTarget->getLevel() > damage)
                    return false;
            break;
    }
    ...
    switch(m_spellInfo->Effect[eff])
    {
        ...
        case SPELL_EFFECT_RESURRECT_NEW:
            ...
            if(pTarget != m_caster && !pTarget->IsWithinLOSInMap(m_caster))
            {
                if(!m_targets.GetCorpseTargetGUID())
                    return false;
                Corpse *corpse = ObjectAccessor::GetCorpse(*m_caster, m_targets.GetCorpseTargetGUID());
                if(!corpse)
                    return false;
                if(pTarget->GetGUID() != corpse->GetOwnerGUID())
                    return false;
        ...
        default:                                            // normal case
            ...
            if(!caster)
                caster = m_caster;
            if(pTarget->GetEntry() == 5925)
                return true;
            if(pTarget != m_caster && !pTarget->IsWithinLOSInMap(caster))
                return false;
            break;
    }

    // Allow to implement hacky pTarget checks for specific spell Id's
    switch(m_spellInfo->Id)
    {
        case 71615:    // Tear Gas (Professor Putricide encounter)
        case 71617:
        case 71618:
            ...
            if(pTarget->GetTypeId() != TYPEID_PLAYER)
                return false;
            break;
        case 70539:    // Regurgitated Ooze (Professor Putricide encounter)
        case 72457:    // Regurgitated Ooze (Professor Putricide encounter)
        case 72875:    // Regurgitated Ooze (Professor Putricide encounter)
        case 72876:    // Regurgitated Ooze (Professor Putricide encounter)
            // Should not affect Professor Putricide
            if(pTarget->GetEntry() == 36678 || pTarget->GetEntry() == 38431 || pTarget->GetEntry() == 38585 || pTarget->GetEntry() == 38586)
                return false;
            break;
        default:
            break;
...
SpellEvent::SpellEvent(Spell* pSpell) : BasicEvent() { m_Spell = pSpell; }

...
void Aura::UpdateTargetMap(Unit* caster, bool apply)
{
    ...
    for(std::map<Unit* , uint8>::iterator itr = targets.begin(); itr!= targets.end();)
    {
        ...
        } // owner has to be in world, or effect has to be applied to self, else skip (when shit happen)
        if(!addUnit)
            targets.erase(itr++);
        else if((!GetOwner()->IsInWorld() && GetOwner() == itr->first) || GetOwner()->IsInMap(itr->first)) {
            itr->first->_CreateAuraApplication(this, itr->second); ++itr;
        } else targets.erase(++itr);
    }
    ...
    if(apply)
    {
        switch(GetSpellProto()->SpellFamilyName)
        {
            ...
            case SPELLFAMILY_HUNTER:
                ...
                if(GetSpellProto()->SpellFamilyFlags[1] & 0x01000000)
                {
                    ...
                    if(AuraEffect* auraEff = target->GetAuraEffect(SPELL_AURA_DUMMY, SPELLFAMILY_HUNTER, 3560, 1))
                    {
                        ...
                        {
                            case 53228:
                                target->CastSpell(target, 56654, true);
                                break;
                            case 53232:
                                target->CastSpell(target, 58882, true);
                                break;
                        }
                ...
                switch(GetId())
                {
                   case 6358: // Seduction
                    ...
                    case 48020: // Demonic Circle
                        if(target->GetTypeId() == TYPEID_PLAYER)
                            if(GameObject* pGo = target->GetGameObject(48018))
                            {
                                target->NearTeleportTo(pGo->GetPositionX(), pGo->GetPositionY(), pGo->GetPositionZ(), pGo->GetOrientation());
            ...
            case SPELLFAMILY_ROGUE:
                if(GetSpellProto()->SpellFamilyFlags[0] & 0x40 && GetSpellProto()->Category == 44) {   // Sprint (skip non player casted spells by category)
                    if(target->HasAura(58039))                 // Glyph of Blurred Speed
                        target->CastSpell(target, 61922, true); // Sprint (waterwalk)
                }

                if(GetSpellProto()->SpellFamilyFlags[1] & 0x80000 && caster) {                          // Savage Combat triggered at poisons apply
    ...
    switch(GetSpellProto()->SpellFamilyName)
    {
        ...
        case SPELLFAMILY_DEATHKNIGHT:
            ...
            if(GetId() == 46619)                                 // Raise ally
            {
                ...
                if(apply) { // convert player to ghoul
                    pTarget->setDeathState(GHOULED);
                    pTarget->SetMovement(MOVE_UNROOT);
                } else {
                    pTarget->SetMovement(MOVE_ROOT);
                    pTarget->SetHealth(0);
...
void DynObjAura::FillTargetMap(std::map<Unit* , uint8> & targets, Unit* /*pCaster*/)
{

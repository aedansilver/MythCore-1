...
public:
    ...
    bool OnGossipHello(Player* player, Creature* creature)
    {
        ...
        if(pInstance)
        {
            ...
            } else player->SEND_GOSSIP_MENU(GOSSIP_ID_START_1, creature->GetGUID());
        }
        return true;
    }
                            ...
                            switch(eventProgress)
                            {
                                ...
                                case 6:
                                    if(pInstance->GetData(TYPE_MUTANUS_THE_DEVOURER) == DONE)
                                    {
                                        eventTimer = 3000;
                                        ++eventProgress;
                                        AchievementEntry const* pAchievement = GetAchievementStore()->LookupEntry(ACHIEVEMENT_WAILING_CAVERNS);
                                        if(pAchievement)
                                        {
                                            Map* pMap = me->GetMap();

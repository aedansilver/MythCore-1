    ...
    struct boss_onyxiaAI : public ScriptedAI
    {
        ...
        void UpdateAI(const uint32 diff)
        {
            ...
            if(m_uiPhase == PHASE_START || m_uiPhase == PHASE_END)
            {
                ...
                {
                    if(m_uiBellowingRoarTimer <= diff)
                    {
                        ...
                        if(pInstance && pFloor)
                            pInstance->SetData64(DATA_FLOOR_ERUPTION_GUID, pFloor->GetGUID());
                        m_uiBellowingRoarTimer = 30000;
                    }
                    else
                        m_uiBellowingRoarTimer -= diff;
                }
                ...
                if(m_uiWingBuffetTimer <= diff)
                {
                    DoCastVictim(SPELL_WING_BUFFET);
                    m_uiWingBuffetTimer = urand(15000, 30000);
                }
                else
                    m_uiWingBuffetTimer -= diff;
            ...
            else
            {
                ...
                if(m_uiFireballTimer <= diff)
                {
                    ...
                    m_uiFireballTimer -= diff;

                if(m_uiLairGuardTimer <= diff)
                {

        ...
        if(pAI && pAI->bStepping == false)
        {
            switch(pAI->uiGossipStep)
            {
                case 0: //This one is a workaround since the very beggining of the script is wrong.
                {
                    ...
                    // If player already completed the pInstance can jump the Jaina/Uther intro
            ...
            void SpellHit(Unit* /*pCaster*/, SpellEntry const* spell)
            {
                if(spell->Id == SPELL_ARCANE_DISRUPTION && !_marked)
                {
                    ...
                    if(InstanceScript* pInstance = me->GetInstanceScript())
                        pInstance->SetData(DATA_CRATE_COUNT, pInstance->GetData(DATA_CRATE_COUNT) + 1);
...
public:
    ...
    bool OnGossipSelect(Player* pPlayer, Creature* /*pCreature*/, uint32 /*sender*/, uint32 action)
    {
        ...
        {
            case GOSSIP_ACTION_INFO_DEF:
                pPlayer->AddItem(ITEM_ARCANE_DISRUPTOR, 1);
                break;
            case GOSSIP_ACTION_INFO_DEF+1:
                pPlayer->TeleportTo(pPlayer->GetMapId(), ChromieEntranceSummonPos.GetPositionX() - 3.0f, ChromieEntranceSummonPos.GetPositionY() - 3.0f, ChromieEntranceSummonPos.GetPositionZ(), 0.0f);
                break;
        }
    ...
    bool OnGossipHello(Player* player, Creature* creature)
    {
        ...
        if(pInstance)
        {
            ...
            if(pInstance->GetData(DATA_CRATE_COUNT) < 5)
            {
...
class npc_cos_zombie : public CreatureScript
{
    public:
        ...
        struct npc_cos_zombieAI : public ScriptedAI
        {
            ...
            {
                if(pInstance)
                    pInstance->SetData(DATA_ZOMBIEFEST, SPECIAL);
            }

...
public:
    ...
    class spell_gen_oracle_wolvar_reputation_SpellScript : public SpellScript
    {
        ...
        void HandleDummy(SpellEffIndex effIndex)
        {
            ...
            {
                uint32 factionId = SpellMgr::CalculateSpellEffectAmount(GetSpellInfo(), effIndex, GetCaster());//uint32 factionId = GetSpellInfo()->Effect[effIndex].CalcValue();
                int32  repChange = SpellMgr::CalculateSpellEffectAmount(GetSpellInfo(), EFFECT_1, GetCaster());//int32  repChange = GetSpellInfo()->Effect[EFFECT_1].CalcValue();

...
public:
    ...
    public:
        ...
        void Register()
        {
            OnEffect += SpellEffectFn(spell_q12372_destabilize_azure_dragonshrine_dummy_SpellScript::HandleDummy, EFFECT_0, SPELL_EFFECT_DUMMY);
        }
    };
...
class spell_summon_nass : public SpellScriptLoader
{
public:
    spell_summon_nass() : SpellScriptLoader("spell_summon_nass") { }

    class spell_summon_nass_SpellScript : public SpellScript
    {
        PrepareSpellScript(spell_summon_nass_SpellScript);

        bool Validate(SpellEntry const* /*spellEntry*/)
        {
            if(!sSpellStore.LookupEntry(SPELL_SUMMON_NASS))
                return false;
            return true;
        }
        ...
        void HandleEffectScriptEffect(SpellEffIndex /*effIndex*/)
        {
            if(Unit* target = GetHitUnit())
                target->CastSpell(target, SPELL_SUMMON_NASS, true);
        }

        void Register()
        {
            OnEffect += SpellEffectFn(spell_summon_nass_SpellScript::HandleEffectScriptEffect, EFFECT_0, SPELL_EFFECT_SCRIPT_EFFECT);
        }
    };

    SpellScript* GetSpellScript() const
    {
        return new spell_summon_nass_SpellScript;
    }
};

enum RivenwoodCaptives
{
    CREATURE_BABY_RIVEN_WIDOW    = 24219,
    CREATURE_LONGRUNNER          = 24211,
    SPELL_SUMMON_LONGRUNNER      = 43289
};

class spell_rivenwood_captives_no_quest : public SpellScriptLoader
{
public:
    spell_rivenwood_captives_no_quest() : SpellScriptLoader("spell_rivenwood_captives_no_quest") { }

    class spell_rivenwood_captives_no_quest_SpellScript : public SpellScript
    {
        PrepareSpellScript(spell_rivenwood_captives_no_quest_SpellScript);

        void HandleDummy(SpellEffIndex /*effIndex*/)
        {
            if(Unit* caster = GetCaster())
                if(Unit* target = GetHitUnit())
                    if(Creature* spider = caster->SummonCreature(CREATURE_BABY_RIVEN_WIDOW, caster->GetPositionX(), caster->GetPositionY(), caster->GetPositionZ(), caster->GetOrientation(), TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 10000))
                        spider->AI()->AttackStart(target);
        }

        void Register()
        {
            OnEffect += SpellEffectFn(spell_rivenwood_captives_no_quest_SpellScript::HandleDummy, EFFECT_0, SPELL_EFFECT_DUMMY);
        }
    };

    SpellScript* GetSpellScript() const
    {
        return new spell_rivenwood_captives_no_quest_SpellScript;
    }
};

class spell_rivenwood_captives : public SpellScriptLoader
{
public:
    spell_rivenwood_captives() : SpellScriptLoader("spell_rivenwood_captives") { }

    class spell_rivenwood_captives_SpellScript : public SpellScript
    {
        PrepareSpellScript(spell_rivenwood_captives_SpellScript);
        ...
        void HandleDummy(SpellEffIndex /*effIndex*/)
        {
            if(Unit* caster = GetCaster()) {
                if(Unit* target = GetHitUnit()) {
                    if(target->ToPlayer())
                    {
                        caster->CastSpell(caster, SPELL_SUMMON_LONGRUNNER, true);
                        target->ToPlayer()->KilledMonsterCredit(CREATURE_LONGRUNNER, 0);
                    }
                }
            }
        }
        ...
        void Register()
        {
            OnEffect += SpellEffectFn(spell_rivenwood_captives_SpellScript::HandleDummy, EFFECT_0, SPELL_EFFECT_DUMMY);
        }
    };

    SpellScript* GetSpellScript() const
    {
        return new spell_rivenwood_captives_SpellScript;
    }
};
    ...
    public:
        ...
        void HandleDummy(SpellEffIndex /*effIndex*/)
        {
            if(Creature* creatureTarget = GetHitCreature())
            {
                ...
                if(uiNewEntry)
                {
                    if(creatureTarget->UpdateEntry(uiNewEntry))
                    {
                        creatureTarget->setFaction(14);
                        Player* player = GetCaster()->ToPlayer();
                        creatureTarget->AddThreat(player, 0.0f);
                        creatureTarget->SetInCombatWith(player);
                    }
                }
            }
        }
...
public:
    ...
    class spell_symbol_of_life_dummy_SpellScript : public SpellScript
    {
        PrepareSpellScript(spell_symbol_of_life_dummy_SpellScript);
          ...
          {
              Unit* caster = GetCaster();
              if(caster->FindNearestCreature(NPC_NARM_FAULK, 30.0f, true)  ||
                  caster->FindNearestCreature(NPC_HENZE_FAULK, 30.0f, true) ||
                  caster->FindNearestCreature(NPC_YOUNG_SHAMAN, 30.0f, true))
        ...
        void Register()
        {
            ...
            OnCheckCast += SpellCheckCastFn(spell_symbol_of_life_dummy_SpellScript::CheckRequirement);
        }
    };
...
// http://www.wowhead.com/quest=10657 Ride the Lightning
enum eQuest10657Data
{
    NPC_RIDE_THE_LIGHTNING_KC_BUNNY  = 21910,
    SPELL_REPOLARIZED_MAGNETO_SPHERE = 37830,
};
class spell_q10657_lightning_strike : public SpellScriptLoader
{
public:
    spell_q10657_lightning_strike() : SpellScriptLoader("spell_q10657_lightning_strike") { }

    class spell_q10657_lightning_strike_SpellScript : public SpellScript
    {
        PrepareSpellScript(spell_q10657_lightning_strike_SpellScript);

        void KillCredit()
        {
            if(Player* target = GetTargetUnit()->ToPlayer())
            {
                if(target->HasAura(SPELL_REPOLARIZED_MAGNETO_SPHERE))
                    target->KilledMonsterCredit(NPC_RIDE_THE_LIGHTNING_KC_BUNNY, 0);
            }
        }

        void Register()
        {
            OnHit += SpellHitFn(spell_q10657_lightning_strike_SpellScript::KillCredit);
        }
    };

    SpellScript* GetSpellScript() const
    {
        return new spell_q10657_lightning_strike_SpellScript();
    };
};

// Quest: Finding the Source http://www.wowhead.com/quest=974
enum temperature_reading
{
    GO_HOT_POINT = 148503,
};

class spell_temperature_reading : public SpellScriptLoader
{
public:
    spell_temperature_reading() : SpellScriptLoader("spell_temperature_reading") { }

    class spell_temperature_reading_SpellScript : public SpellScript
    {
        PrepareSpellScript(spell_temperature_reading_SpellScript);
        ...
        void HandleDummy(SpellEffIndex /*effIndex*/)
        {
            ...
            if(GameObject* go = caster->FindNearestGameObject(GO_HOT_POINT, 10.0f))
            {
                if(go->GetPositionZ() == -188 && caster->GetQuestStatus(974) == QUEST_STATUS_INCOMPLETE)
                    caster->KilledMonsterCredit(10541, 0);
            }
        }

        void Register()
        {
            OnEffect += SpellEffectFn(spell_temperature_reading_SpellScript::HandleDummy, EFFECT_0, SPELL_EFFECT_DUMMY);
        }
    };

    SpellScript* GetSpellScript() const
    {
        return new spell_temperature_reading_SpellScript();
    };
};

class spell_returning_vrykul_artifact : public SpellScriptLoader
{
public:
    spell_returning_vrykul_artifact() : SpellScriptLoader("spell_returning_vrykul_artifact") { }

    class spell_returning_vrykul_artifact_SpellScript : public SpellScript
    {
        PrepareSpellScript(spell_returning_vrykul_artifact_SpellScript);

        bool Validate(SpellEntry const* /*spellEntry*/)
        {
            if(!sSpellStore.LookupEntry(45323))
                return false;
            return true;
        }
        ...
        void HandleDummy(SpellEffIndex /*effIndex*/)
        {
            ...
            if(pCaster->GetTypeId() == TYPEID_PLAYER)
            {
                ...
                if(pPlayer->GetQuestStatus(11568) == QUEST_STATUS_INCOMPLETE)
                {
                    if(pPlayer->FindNearestCreature(25232, 30.0f, true))
                        pPlayer->KilledMonsterCredit(24874, 0);
                    else if(pPlayer->FindNearestCreature(25231, 30.0f, true))
                        pPlayer->KilledMonsterCredit(24876, 0);
                    else if(pPlayer->FindNearestCreature(25229, 30.0f, true))
                        pPlayer->KilledMonsterCredit(24877, 0);
                    else if(pPlayer->FindNearestCreature(25230, 30.0f, true))
                        pPlayer->KilledMonsterCredit(24875, 0);
                }
            }
        }
        ...
        void Register()
        {
            OnEffect += SpellEffectFn(spell_returning_vrykul_artifact_SpellScript::HandleDummy, EFFECT_0, SPELL_EFFECT_DUMMY);
        }
    };

    SpellScript* GetSpellScript() const
    {
        return new spell_returning_vrykul_artifact_SpellScript();
    }
};
...
void AddSC_quest_spell_scripts()
{
    ...
    new spell_summon_nass;
    new spell_rivenwood_captives_no_quest;
    new spell_rivenwood_captives;
    ...
    new spell_q10657_lightning_strike;
    new spell_temperature_reading;
    new spell_returning_vrykul_artifact;
}

...
class go_crown_chemical_supplies : public GameObjectScript
{
public:
    go_crown_chemical_supplies() : GameObjectScript("go_crown_chemical_supplies") { }

    bool OnGossipHello(Player* pPlayer, GameObject* /*pGo*/)
    {
        if(pPlayer->GetQuestStatus(24541) == QUEST_STATUS_INCOMPLETE)
            pPlayer->CastSpell(pPlayer, 71539, true);
        else if(pPlayer->GetQuestStatus(24656) == QUEST_STATUS_INCOMPLETE)
            pPlayer->CastSpell(pPlayer, 71522, true);
        return false;
    }
};
...
void AddSC_go_scripts()
{
    ...
    new go_crown_chemical_supplies;
}

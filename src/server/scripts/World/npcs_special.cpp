        ...
        void SpellHit(Unit* pHitter, const SpellEntry* pSpell)
        {
            if(pSpell->Id == SPELL_LESSER_HEAL_R2 || pSpell->Id == SPELL_FORTITUDE_R1)
            {
                ...
                if(pHitter->GetTypeId() == TYPEID_PLAYER)
                {
                    ...
                    {
                        case ENTRY_SHAYA:
                            if(CAST_PLR(pHitter)->GetQuestStatus(QUEST_MOON) == QUEST_STATUS_INCOMPLETE)
                            {
                                if(bIsHealed && !bCanRun && pSpell->Id == SPELL_FORTITUDE_R1)
                                {
                                    DoScriptText(SAY_SHAYA_THANKS, me, pHitter);
                                    bCanRun = true;
                                }
                                else if(!bIsHealed && pSpell->Id == SPELL_LESSER_HEAL_R2)
                                {
                                    caster = pHitter->GetGUID();
                                    me->SetStandState(UNIT_STAND_STATE_STAND);
                                    DoScriptText(SAY_COMMON_HEALED, me, pHitter);
                                    bIsHealed = true;
                                }
                            }
                            break;
                        case ENTRY_ROBERTS:
                            if(CAST_PLR(pHitter)->GetQuestStatus(QUEST_LIGHT_1) == QUEST_STATUS_INCOMPLETE)
                            {
                                if(bIsHealed && !bCanRun && pSpell->Id == SPELL_FORTITUDE_R1)
                                {
                                    DoScriptText(SAY_ROBERTS_THANKS, me, pHitter);
                                    bCanRun = true;
                                }
                                else if(!bIsHealed && pSpell->Id == SPELL_LESSER_HEAL_R2)
                                {
                            ...
                            break;
                        case ENTRY_DOLF:
                            if(CAST_PLR(pHitter)->GetQuestStatus(QUEST_LIGHT_2) == QUEST_STATUS_INCOMPLETE)
                            {
                                if(bIsHealed && !bCanRun && pSpell->Id == SPELL_FORTITUDE_R1)
                                {
                                    DoScriptText(SAY_DOLF_THANKS, me, pHitter);
                                    bCanRun = true;
                                }
                                else if(!bIsHealed && pSpell->Id == SPELL_LESSER_HEAL_R2)
                                {
                            ...
                            break;
                        case ENTRY_KORJA:
                            if(CAST_PLR(pHitter)->GetQuestStatus(QUEST_SPIRIT) == QUEST_STATUS_INCOMPLETE)
                            {
                                if(bIsHealed && !bCanRun && pSpell->Id == SPELL_FORTITUDE_R1)
                                {
                                    DoScriptText(SAY_KORJA_THANKS, me, pHitter);
                                    bCanRun = true;
                                }
                                else if(!bIsHealed && pSpell->Id == SPELL_LESSER_HEAL_R2)
                                {
                            ...
                            break;
                        case ENTRY_DG_KEL:
                            if(CAST_PLR(pHitter)->GetQuestStatus(QUEST_DARKNESS) == QUEST_STATUS_INCOMPLETE)
                            {
                                if(bIsHealed && !bCanRun && pSpell->Id == SPELL_FORTITUDE_R1)
                                {
                                    DoScriptText(SAY_DG_KEL_THANKS, me, pHitter);
                                    bCanRun = true;
                                }
                                else if(!bIsHealed && pSpell->Id == SPELL_LESSER_HEAL_R2)
                                {
                    ...
                    if(bCanRun)
                        CAST_PLR(pHitter)->TalkedToCreature(me->GetEntry(), me->GetGUID());
                }
            }
        }
...
public:
    ...
    struct npc_ebon_gargoyleAI : CasterAI
    {
        ...
        void InitializeAI()
        {
            ...
            // TODO: would it be faster to use owner->ToPlayer()->GetSelectedUnit()? or something with last spellcast?
        ...
        void JustDied(Unit* /*pKiller*/)
        {
            ...
            if(Unit* owner = me->GetOwner())
                owner->RemoveAura(61777);
            if(Unit *victim = me->getVictim())
                victim->RemoveAura(49206);
        }
...
class npc_captain_demeza : public CreatureScript
{
public:
    npc_captain_demeza() : CreatureScript("npc_captain_demeza") { }
    ...
    bool OnGossipSelect(Player* player, Creature* /*pCreature*/, uint32 /*uiSender*/, uint32 uiAction)
    {
        ...
        if(uiAction == GOSSIP_ACTION_INFO_DEF+1)
        {
            if(AchievementEntry const* achievement = GetAchievementStore()->LookupEntry(3457))
    ...
    bool OnGossipHello(Player* player, Creature* creature)
    {
        player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "Tomar una cerveza!", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF+1);
        player->SEND_GOSSIP_MENU(player->GetGossipTextId(creature), creature->GetGUID());
...
class npc_brewfest_achievement_manager : public CreatureScript
{
public:
    npc_brewfest_achievement_manager() : CreatureScript("npc_brewfest_achievement_manager") { }

    bool OnQuestReward(Player* player, Creature* /*pCreature*/, Quest const* quest, uint32 /*opt*/)
    {
        if(quest->GetQuestId() == 12421 || quest->GetQuestId() == 12420)
        {
            if(AchievementEntry const* achievement = GetAchievementStore()->LookupEntry(2796))
                player->CompletedAchievement(achievement);
        }
...
public:
    ...
    bool OnQuestReward(Player* player, Creature* /*pCreature*/, Quest const* quest, uint32 /*opt*/)
    {
        if(quest->GetQuestId() == 12135 || quest->GetQuestId() == 11131)
        {
            if(player->GetQuestStatus(12135) == QUEST_STATUS_REWARDED && player->GetQuestStatus(11131) == QUEST_STATUS_REWARDED)
            {
                if(AchievementEntry const* achievement = GetAchievementStore()->LookupEntry(289))
                    player->CompletedAchievement(achievement);
            }
        } else if(quest->GetQuestId() == 12139 || quest->GetQuestId() == 11219) {
            if(player->GetQuestStatus(12139) == QUEST_STATUS_REWARDED && player->GetQuestStatus(11219) == QUEST_STATUS_REWARDED)
            {
                if(AchievementEntry const* achievement = GetAchievementStore()->LookupEntry(289))
                    player->CompletedAchievement(achievement);
            }
...
public:
    ...
    {
        return new npc_eversong_rangerAI(pCreature);
    }
};

class npc_snip_snagglebolt : public CreatureScript
{
public:
    npc_snip_snagglebolt() : CreatureScript("npc_snip_snagglebolt") { }

    bool OnQuestAccept(Player* /*pPlayer*/, Creature* pCreature, Quest const* pQuest)
    {
        if(pQuest->GetQuestId() == 24536)
        {
            std::list<Creature*> pGuards;
            GetCreatureListWithEntryInGrid(pGuards, pCreature, 3296, 150.0f);

            if(pGuards.empty())
                return false;

            for(std::list<Creature*>::iterator iter = pGuards.begin(); iter != pGuards.end(); ++iter)
            {
                if(SpellEntry const *spellInfo = sSpellStore.LookupEntry(71507))
                    Aura::TryRefreshStackOrCreate(spellInfo, MAX_EFFECT_MASK, (*iter), (*iter));
            }
        } else if(pQuest->GetQuestId() == 24655) {
            std::list<Creature*> pGuards;
            GetCreatureListWithEntryInGrid(pGuards, pCreature, 68, 150.0f);
    ...
    {
        if((pQuest->GetQuestId() == 24536 || pQuest->GetQuestId() == 24655) && pPlayer->HasAura(62061))
            pPlayer->RemoveAura(71507);
        return true;
    }
};
...
void AddSC_npcs_special()
{
    ...
    new npc_experience;
    new npc_captain_demeza;
    new npc_brewfest_achievement_manager;
    new npc_halloween_orphan_matron;
    new npc_eversong_ranger;
    new npc_snip_snagglebolt;
}
...
enum eBoxOfTricks
{
    QUEST_TO_LEGION_HOLD        = 10563,
    NPC_IMAGE_WARBRINGER_RAZUUN = 21502
};

class item_box_of_tricks : public ItemScript
{
public:
    item_box_of_tricks() : ItemScript("item_box_of_tricks") { }

    bool OnUse(Player* pPlayer, Item* /*pItem*/, SpellCastTargets const& /*targets*/)
    {
        if(pPlayer->GetQuestStatus(QUEST_TO_LEGION_HOLD) == QUEST_STATUS_INCOMPLETE)
            pPlayer->KilledMonsterCredit(NPC_IMAGE_WARBRINGER_RAZUUN, 0);
        return true;
    }
};

...
public:
    ...
    struct boss_halion_realAI : public ScriptedAI
    {
        ...
        void setStage(uint8 phase) { stage = phase; }
        ...
        void UpdateAI(const uint32 diff)
        {
            ...
            if(m_uiCorporealityTimer <= diff)
            {
                ...
                if(m_lastCorpReal - Buff[buffnum].disp_corp < 0) {
                    DoScriptText(EMOTE_REAL, pHalionReal);
                    DoScriptText(EMOTE_TWILIGHT, pHalionTwilight);
...
public:
    ...
    struct mob_halion_orbAI : public ScriptedAI
    {
        ...
        void StartMovement(uint32 id)
        {
            ...
            if(Creature* pFocus = me->GetMap()->GetCreature(pInstance->GetData64(NPC_ORB_ROTATION_FOCUS)))
                ...
                if(Player* pPlayer = i->getSource())
                {
                    if(pPlayer->isAlive() && pPlayer->HasAura(SPELL_MARK_OF_CONSUMPTION))
                    {
                        ...
                        if(!grow)
                        {
                            ...
                            pInstance->DoRemoveAurasDueToSpellOnPlayers(SPELL_MARK_OF_CONSUMPTION);
                            grow = true;
                        }
                    }
                }
                ...
                if(Player* pPlayer = i->getSource())
                {
                    if(pPlayer->isAlive() && pPlayer->HasAura(SPELL_MARK_OF_COMBUSTION))
                    {
                        ...
                        if(!grow)
                        {
                            ...
                            pInstance->DoRemoveAurasDueToSpellOnPlayers(SPELL_MARK_OF_COMBUSTION);
                            grow = true;
                        }
                    }
                }
...
public:
    ...
    struct mob_halion_meteorAI : public ScriptedAI
    {
        ...
        void UpdateAI(const uint32 diff)
        {
            ...
            {
                case 0:
                    if(m_uiMeteorImpactTimer <= diff)
                    {
                        DoCast(SPELL_METEOR_IMPACT);
                        m_uiMeteorImpactTimer = 500;
                        setStage(1);
                    } else m_uiMeteorImpactTimer -= diff;
                    break;
                case 1:

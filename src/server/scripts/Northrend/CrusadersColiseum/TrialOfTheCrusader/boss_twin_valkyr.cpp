...
struct boss_twin_baseAI : public ScriptedAI
{
    ...
    void JustReachedHome()
    {
        if(pInstance)
        {
            ...
            pInstance->SetData(DATA_HEALTH_TWIN_SHARED, me->GetMaxHealth());
        }
                ...
                for(Map::PlayerList::const_iterator itr = lPlayers.begin(); itr != lPlayers.end(); ++itr)
                {
                    ...
                    if(pPlayer->isAlive())
                    {
                        if(pSummoned->GetEntry() == NPC_LIGHT_ESSENCE)
                            pPlayer->RemoveAurasDueToSpell(RAID_MODE(SPELL_LIGHT_ESSENCE_10_N, SPELL_LIGHT_ESSENCE_25_N, SPELL_LIGHT_ESSENCE_10_H, SPELL_LIGHT_ESSENCE_25_H));
                        if(pSummoned->GetEntry() == NPC_DARK_ESSENCE)
                            pPlayer->RemoveAurasDueToSpell(RAID_MODE(SPELL_DARK_ESSENCE_10_N, SPELL_DARK_ESSENCE_25_N, SPELL_DARK_ESSENCE_10_H, SPELL_DARK_ESSENCE_25_H));
                    }
                }
                break;
        }
    ...
    void DamageTaken(Unit* pDoneBy, uint32 &uiDamage)
    {
        ...
        if(pInstance)
            pInstance->SetData(DATA_HEALTH_TWIN_SHARED, me->GetHealth() >= uiDamage ? me->GetHealth() - uiDamage : 0);
    }
    ...
    void SpellHit(Unit* caster, const SpellEntry* spell)
    {
        if(caster->ToCreature() == me)
            if(spell->Effect[0] == 136) //Effect Heal
                if(pInstance)
                    pInstance->SetData(DATA_HEALTH_TWIN_SHARED, me->GetHealth() + me->CountPctFromMaxHealth(spell->EffectBasePoints[0]));
    }
    ...
    Creature* GetSister()
    {
        return Unit::GetCreature((*me), pInstance->GetData64(m_uiSisterNpcId));
    }
...
public:
    ...
    struct mob_unleashed_darkAI : public mob_unleashed_ballAI
    {
        ...
        {
            if(pSpell->Id == (uint8)RAID_MODE(SPELL_UNLEASHED_DARK_10, SPELL_UNLEASHED_DARK_25, SPELL_UNLEASHED_DARK_10, SPELL_UNLEASHED_DARK_25)
            && pWho->HasAura(RAID_MODE(SPELL_DARK_ESSENCE_10_N, SPELL_DARK_ESSENCE_25_N, SPELL_DARK_ESSENCE_10_H, SPELL_DARK_ESSENCE_25_H)))
            {
                ...
                if(Aura * aura = pWho->GetAura(RAID_MODE(SPELL_POWERING_UP_10_N, SPELL_POWERING_UP_25_N, SPELL_POWERING_UP_10_H, SPELL_POWERING_UP_25_H)))
                {
                    ...
                    } else aura->SetStackAmount(aura->GetStackAmount()+n);
                }
            }
        }
            ...
            if(m_uiRangeCheckTimer < diff)
            {
                if(Unit* pTarget = me->SelectNearestTarget(2.0f))
                    if(pTarget->GetTypeId() == TYPEID_PLAYER && pTarget->isAlive())
                    {
                        ...
                        if(pTarget->HasAura(RAID_MODE(SPELL_DARK_ESSENCE_10_N, SPELL_DARK_ESSENCE_25_N, SPELL_DARK_ESSENCE_10_H, SPELL_DARK_ESSENCE_25_H)))
                            me->CastSpell(pTarget, RAID_MODE(SPELL_POWERING_UP_10_N, SPELL_POWERING_UP_25_N, SPELL_POWERING_UP_10_H, SPELL_POWERING_UP_25_H), true);
                        me->GetMotionMaster()->MoveIdle();
                        me->DespawnOrUnsummon(1000);
                    }
                m_uiRangeCheckTimer = IN_MILLISECONDS;
            }
            else m_uiRangeCheckTimer -= diff;
        }
    };
};
...
public:
    ...
    struct mob_unleashed_lightAI : public mob_unleashed_ballAI
    {
        ...
        void SpellHitTarget(Unit* pWho, const SpellEntry* pSpell)
        {
            if(pSpell->Id == (uint8)RAID_MODE(SPELL_UNLEASHED_LIGHT_10, SPELL_UNLEASHED_LIGHT_25, SPELL_UNLEASHED_LIGHT_10, SPELL_UNLEASHED_LIGHT_25)
            ...
            if(m_uiRangeCheckTimer < diff)
            {
                if(Unit* pTarget = me->SelectNearestTarget(2.0f))
                    if(pTarget->GetTypeId() == TYPEID_PLAYER && pTarget->isAlive())
                    {
                        ...
                        if(pTarget->GetAura(RAID_MODE(SPELL_DARK_ESSENCE_10_N, SPELL_DARK_ESSENCE_25_N, SPELL_DARK_ESSENCE_10_H, SPELL_DARK_ESSENCE_25_H)))
                            me->CastSpell(pTarget, RAID_MODE(SPELL_POWERING_UP_10_N, SPELL_POWERING_UP_25_N, SPELL_POWERING_UP_10_H, SPELL_POWERING_UP_25_H), true);
                        me->GetMotionMaster()->MoveIdle();
                        me->DespawnOrUnsummon(1000);
                    }

...
enum BossSpells
{
    ...
    SPELL_PENETRATING_COLD_10_N  = 66013,
    SPELL_PENETRATING_COLD_25_N  = 67700,
    SPELL_PENETRATING_COLD_10_H  = 68509,
    SPELL_PENETRATING_COLD_25_H  = 68510,
    SPELL_LEECHING_SWARM_10_N    = 66118,
    SPELL_LEECHING_SWARM_25_N    = 67630,
    SPELL_LEECHING_SWARM_10_H    = 68646,
...
public:
    ...
    struct boss_anubarak_trialAI : public ScriptedAI
    {
        ...
        {
            if(pInstance)
                pInstance->SetData(TYPE_ANUBARAK, FAIL);
        ...
        void UpdateAI(const uint32 diff)
        {
            ...
            {
                case 0:
                    if(m_uiFreezeSlashTimer <= diff)
                    {
                        DoCastVictim(SPELL_FREEZE_SLASH);
                        m_uiFreezeSlashTimer = 15*IN_MILLISECONDS;
                    } else m_uiFreezeSlashTimer -= diff;

                    if(m_uiPenetratingColdTimer <= diff)
                    {
                        me->CastCustomSpell(RAID_MODE(SPELL_PENETRATING_COLD_10_N, SPELL_PENETRATING_COLD_25_N, SPELL_PENETRATING_COLD_10_H, SPELL_PENETRATING_COLD_25_H), SPELLVALUE_MAX_TARGETS, RAID_MODE(2, 5, 2, 5));
                        m_uiPenetratingColdTimer = 20*IN_MILLISECONDS;
                    } else m_uiPenetratingColdTimer -= diff;
                    ...
                    if(IsHeroic() && m_uiNerubianShadowStrikeTimer <= diff)
                    {
                        Summons.DoAction(NPC_BURROWER, ACTION_SHADOW_STRIKE);
                        m_uiNerubianShadowStrikeTimer = 30*IN_MILLISECONDS;
                    } else m_uiNerubianShadowStrikeTimer -= diff;
...
public:
    ...
    struct mob_nerubian_burrowerAI : public ScriptedAI
    {
        ...
        void UpdateAI(const uint32 diff)
        {
            ...
            if((m_uiSubmergeTimer <= diff) && HealthBelowPct(80))
            {
...
public:
    ...
    struct mob_anubarak_spikeAI : public ScriptedAI
    {
        ...
        void UpdateAI(const uint32 diff)
        {
            ...
            {
                if(Creature* pAnubarak = Unit::GetCreature((*me), pInstance->GetData64(NPC_ANUBARAK)))

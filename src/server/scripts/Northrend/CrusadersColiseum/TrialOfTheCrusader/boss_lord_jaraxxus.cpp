...
public:
    ...
    struct boss_jaraxxusAI : public ScriptedAI
    {
        ...
        {
            if(pInstance)
                pInstance->SetData(TYPE_JARAXXUS, FAIL);
        ...
        void UpdateAI(const uint32 diff)
        {
            ...
            if(m_uiSummonInfernalEruptionTimer <= diff)
            {
                ...
                m_uiSummonInfernalEruptionTimer = 2*MINUTE*IN_MILLISECONDS;
            } else m_uiSummonInfernalEruptionTimer -= diff;

            if(m_uiSummonNetherPortalTimer <= diff)
            {
            ...
            } else m_uiSummonNetherPortalTimer -= diff;

            if(m_uiFelFireballTimer <= diff)
            {
            ...
            } else m_uiFelFireballTimer -= diff;

            if(m_uiFelLightningTimer <= diff)
            {
                ...
                m_uiFelLightningTimer = urand(10*IN_MILLISECONDS, 15*IN_MILLISECONDS);
            } else m_uiFelLightningTimer -= diff;

            if(m_uiIncinerateFleshTimer <= diff)
            {
                ...
                m_uiIncinerateFleshTimer = urand(20*IN_MILLISECONDS, 25*IN_MILLISECONDS);
            } else m_uiIncinerateFleshTimer -= diff;

            if(m_uiNetherPowerTimer <= diff)
            {
                me->SetAuraStack(SPELL_NETHER_POWER, me, RAID_MODE(5, 10, 5, 10));
                m_uiNetherPowerTimer = 40*IN_MILLISECONDS;
            } else m_uiNetherPowerTimer -= diff;

            if(m_uiLegionFlameTimer <= diff)
            {
                ...
                m_uiLegionFlameTimer = 30*IN_MILLISECONDS;
            } else m_uiLegionFlameTimer -= diff;

            if(GetDifficulty() == RAID_DIFFICULTY_25MAN_HEROIC && m_uiTouchOfJaraxxusTimer <= diff)
            {
                if(Unit* pTarget = SelectTarget(SELECT_TARGET_RANDOM, 0))
                    DoCast(pTarget, SPELL_TOUCH_OF_JARAXXUS);
                m_uiTouchOfJaraxxusTimer = urand(10*IN_MILLISECONDS, 15*IN_MILLISECONDS);
            } else m_uiTouchOfJaraxxusTimer -= diff;
...
public:
    ...
    struct mob_fel_infernalAI : public ScriptedAI
    {
        ...
        void UpdateAI(const uint32 diff)
        {
            if(pInstance && pInstance->GetData(TYPE_JARAXXUS) != IN_PROGRESS)
                me->DespawnOrUnsummon();

            if(!UpdateVictim())
                return;

            if(m_uiFelStreakTimer <= diff)
            {
                if(Unit* pTarget = SelectTarget(SELECT_TARGET_RANDOM, 0))
                    DoCast(pTarget, SPELL_FEL_STREAK);
                m_uiFelStreakTimer = 30*IN_MILLISECONDS;
            } else m_uiFelStreakTimer -= diff;
...
public:
    ...
    struct mob_mistress_of_painAI : public ScriptedAI
    {
        mob_mistress_of_painAI(Creature* pCreature) : ScriptedAI(pCreature)
        {
            pInstance = (InstanceScript*)pCreature->GetInstanceScript();
            if(pInstance)
                pInstance->SetData(DATA_MISTRESS_OF_PAIN_COUNT, INCREASE);
            Reset();
        }
        ...
        {
            if(pInstance)
                pInstance->SetData(DATA_MISTRESS_OF_PAIN_COUNT, DECREASE);
        }

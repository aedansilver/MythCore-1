...
public:
    ...
    struct instance_trial_of_the_champion_InstanceMapScript : public InstanceScript
    {
        ...
        void OnCreatureCreate(Creature* pCreature)
        {
            ...
            switch(pCreature->GetEntry())
            {
                ...
                case NPC_ORGRIMAR_CHAMPION:
                    if(TeamInInstance == HORDE)
                        pCreature->UpdateEntry(NPC_STORMWIND_CHAMPION, ALLIANCE);
                    break;
                case NPC_SILVERMOON_CHAMPION:
                    if(TeamInInstance == HORDE)
                        pCreature->UpdateEntry(NPC_GNOMERAGN_CHAMPION, ALLIANCE);
                    break;
                case NPC_THUNDER_CHAMPION:
                    if(TeamInInstance == HORDE)
                        pCreature->UpdateEntry(NPC_EXODAR_CHAMPION, ALLIANCE);
                    break;
                case NPC_TROLL_CHAMPION:
                    if(TeamInInstance == HORDE)
                        pCreature->UpdateEntry(NPC_DRNASSUS_CHAMPION, ALLIANCE);
                    break;
                case NPC_UNDERCITY_CHAMPION:
                    if(TeamInInstance == HORDE)
                        pCreature->UpdateEntry(NPC_IRONFORGE_CHAMPION, ALLIANCE);
                    break;
        ...
        void SetData64(uint32 uiType, uint64 uiData)
        {
            switch(uiType)
            {
                case DATA_GRAND_CHAMPION_1: uiGrandChampion1GUID = uiData;  break;
                case DATA_GRAND_CHAMPION_2: uiGrandChampion2GUID = uiData;  break;
                case DATA_GRAND_CHAMPION_3: uiGrandChampion3GUID = uiData;  break;
            }
        }

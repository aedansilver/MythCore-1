...
class jaina_or_sylvana_intro_HOR : public CreatureScript
{
public:
    jaina_or_sylvana_intro_HOR() : CreatureScript("npc_jaina_and_sylvana_HRintro") { }
    ...
    bool OnGossipSelect(Player* pPlayer, Creature* pCreature, uint32 /*uiSender*/, uint32 uiAction)
    {
        ...
        switch(uiAction)
        {
            case GOSSIP_ACTION_INFO_DEF+1:
                ...
                pInstance->SetData(TYPE_EVENT, 1);
                break;
            case GOSSIP_ACTION_INFO_DEF+2:
                pPlayer->CLOSE_GOSSIP_MENU();
                pInstance->SetData(TYPE_EVENT, 2);
                break;
        }
    ...
    struct jaina_or_sylvana_intro_HORAI : public ScriptedAI
    {
        jaina_or_sylvana_intro_HORAI(Creature* pCreature) : ScriptedAI(pCreature)
        {
        ...
        void StartEvent()
        {
            ...
            pInstance->SetData(TYPE_PHASE, 1);
            ...
            switch(Step)
            {
                ...
                case 2:
                    if(me->GetEntry() == NPC_JAINA) {
                        ...
                        JumpNextStep(5000);
                    } else if(me->GetEntry() == NPC_SYLVANA){
                ...
                case 8:
                    ...
                    m_uiFrostmourneGUID = pInstance->GetData64(GO_FROSTMOURNE);
                    if(GameObject* pFrostmourne = pInstance->instance->GetGameObject(m_uiFrostmourneGUID))
                ...
                case 11:
                    if(me->GetEntry() == NPC_JAINA && pUther) {
                        ...
                        JumpNextStep(7000);
                    } else if(me->GetEntry() == NPC_SYLVANA && pUther) {
                ...
                case 24:
                    ...
                    m_uiMainGateGUID = pInstance->GetData64(GO_IMPENETRABLE_DOOR);
                    if(GameObject* pGate = pInstance->instance->GetGameObject(m_uiMainGateGUID))
                ...
                case 32:
                    ...
                    break;
                case 33:
                    m_uiFalricGUID = pInstance->GetData64(NPC_FALRIC);
                    m_uiMarwynGUID = pInstance->GetData64(NPC_MARWYN);
                    if(Creature* pFalric = pInstance->instance->GetCreature(m_uiFalricGUID)) {
                    ...
                    if(Creature* pMarwyn = pInstance->instance->GetCreature(m_uiMarwynGUID)) {
                    ...
                    break;
                case 34:
                    if(Creature* pFalric = ((Creature*)Unit::GetUnit(*me, m_uiFalricGUID)))
                        DoScriptText(SAY_FALRIC_INTRO, pFalric);
                    if(Creature* pMarwyn = ((Creature*)Unit::GetUnit(*me, m_uiMarwynGUID)))
                ...
                case 37:
                    ...
                    else if(me->GetEntry() == NPC_SYLVANA && pLichKing)
        ...
        void UpdateAI(const uint32 diff)
        {
            ...
            if(pInstance->GetData(TYPE_EVENT) == 1 &&  pInstance->GetData64(DATA_ESCAPE_LIDER) == me->GetGUID())
                StartEvent();

            if(pInstance->GetData(TYPE_EVENT) == 2 &&  pInstance->GetData64(DATA_ESCAPE_LIDER) == me->GetGUID())
            {
                Small = true;
                StartEvent();
            }

            if(StepTimer < diff && pInstance->GetData(TYPE_PHASE) == 1)
                Event();
            else StepTimer -= diff;
    ...
    {
        return new jaina_or_sylvana_intro_HORAI(pCreature);
    }
};

class jaina_or_sylvana_extro_HOR : public CreatureScript
{
public:
    jaina_or_sylvana_extro_HOR() : CreatureScript("npc_jaina_and_sylvana_HRextro") { }
    ...
    bool OnGossipSelect(Player* pPlayer, Creature* pCreature, uint32 /*uiSender*/, uint32 uiAction)
    {
        ...
        switch(uiAction)
        {
            case GOSSIP_ACTION_INFO_DEF+1:
                pPlayer->CLOSE_GOSSIP_MENU();
                ((jaina_or_sylvana_extro_HORAI*)pCreature->AI())->Start(false, true);
    ...
    struct jaina_or_sylvana_extro_HORAI : public npc_escortAI
    {
        jaina_or_sylvana_extro_HORAI(Creature* pCreature) : npc_escortAI(pCreature)
        {
            pInstance = (InstanceScript*)pCreature->GetInstanceScript();
            Reset();
        }
        ...
        void AttackStart(Unit* pWho)
        {
            ...
            if(pInstance->GetData(TYPE_LICH_KING) == IN_PROGRESS || Fight != true)
                return;
        ...
        void WaypointReached(uint32 i)
        {
            switch(i)
            {
                ...
                case 6:
                    ...
                    if(Creature* pWallTarget = pInstance->instance->GetCreature(m_uipWallTargetGUID)) {
        ...
        void MoveInLineOfSight(Unit* pWho)
        {
            ...
            if(me->IsWithinDistInMap(pWho, 50.0f) && pInstance->GetData(TYPE_FROST_GENERAL) == DONE && pInstance->GetData(TYPE_PHASE) == 3)
            {
                pPlayer = (Player* )pWho;
                Event = true;
        ...
        void DamageTaken(Unit* /*pDoneBy*/, uint32 &uiDamage)
        {
            ...
            if(pInstance->GetData(TYPE_LICH_KING) == IN_PROGRESS && WallCast == true)
            {
        ...
        void Intro()
        {
            switch(Step)
            {
                case 0:
                    me->RemoveUnitMovementFlag(MOVEMENTFLAG_WALKING);
                    EncounterTime = 0;
                    m_uiLichKingGUID = pInstance->GetData64(BOSS_LICH_KING);
                    pLichKing = pInstance->instance->GetCreature(m_uiLichKingGUID);
                    JumpNextStep(100);
                    break;
                case 1:
                    if(pLichKing) {
        ...
        void Outro()
        {
            switch(Step)
            {
                ...
                case 15:
                    ...
                    if(pInstance) {
                        if(IsHeroic()) {
                            pInstance->DoCompleteAchievement(ACHIEV_HOR_H);
                            if(EncounterTime <= MAX_ENCOUNTER_TIME)
                                pInstance->DoCompleteAchievement(ACHIEV_NOT_RETREATING);
                        } else pInstance->DoCompleteAchievement(ACHIEV_HOR);
                    }
                    break;
            }
        }
        ...
        void UpdateEscortAI(const uint32 diff)
        {
            ...
            if(pInstance->GetData(TYPE_LICH_KING) == SPECIAL && pInstance->GetData(TYPE_PHASE) != 6) {
            ...
            if(WallCast == true && HoldTimer < 10000 && ( pInstance->GetData(DATA_SUMMONS) == 0 || !me->isInCombat()))
            {
                WallCast = false;
                me->InterruptNonMeleeSpells(false);
                SetEscortPaused(false);
                if(GameObject* pGate = pInstance->instance->GetGameObject(m_uiIceWallGUID))
                    pGate->SetGoState(GO_STATE_ACTIVE);
                ++Count;
                switch(Count)
                {
    ...
    CreatureAI* GetAI(Creature* pCreature) const
    {
        return new jaina_or_sylvana_extro_HORAI(pCreature);
    }
};
...
public:
    ...
    struct npc_frostworn_generalAI : public ScriptedAI
    {
        ...
        void MoveInLineOfSight(Unit* pWho)
        {
            ...
            if(pWho->GetTypeId() != TYPEID_PLAYER || pInstance->GetData(TYPE_MARWYN) != DONE || !me->IsWithinDistInMap(pWho, 20.0f)) return;
            ...
            while(uint32 eventId = events.ExecuteEvent())
            {
                switch(eventId)
                {
                    ...
                    case EVENT_DARK_MENDING:
                        ...
                        Unit* pTarget = DoSelectLowestHpFriendly(40, DUNGEON_MODE(30000,50000));
                        if(pTarget)
                        {
                            DoCast(pTarget, SPELL_DARK_MENDING);
                            events.ScheduleEvent(EVENT_DARK_MENDING, 20000);
                        } // no friendly unit with missing hp. re-check in just 5 sec.
                            else  events.ScheduleEvent(EVENT_DARK_MENDING, 5000);
...
{
    new jaina_or_sylvana_intro_HOR;
    new jaina_or_sylvana_extro_HOR;

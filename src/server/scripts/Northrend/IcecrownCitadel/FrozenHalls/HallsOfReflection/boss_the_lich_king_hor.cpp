...
public:
    ...
    struct boss_lich_king_horAI : public npc_escortAI
    {
        ...
        {
            if(!pInstance || !pSummoned) return;

            pSummoned->SetPhaseMask(65535, true);
            pSummoned->SetInCombatWithZone();
            pSummoned->setActive(true);

            pInstance->SetData(DATA_SUMMONS, 1);

            Unit* pLider = Unit::GetUnit((*me), pInstance->GetData64(DATA_ESCAPE_LIDER));
            if(pLider)
                pSummoned->AddThreat(pLider, 100.0f);
        }
        ...
        void UpdateEscortAI(const uint32 diff)
        {
            ...
            if(pInstance->GetData(TYPE_LICH_KING) == IN_PROGRESS && StartEscort != true)
            {
                StartEscort = true;
        ...
        void UpdateAI(const uint32 diff)
        {
            ...
            if(pInstance->GetData(TYPE_LICH_KING) == IN_PROGRESS)
            {
                if(Emerge != true)
                {
                    ...
                    {
                        Emerge = true;
                        m_uiLiderGUID = pInstance->GetData64(DATA_ESCAPE_LIDER);

...
class boss_lady_deathwhisper : public CreatureScript
{
    public:
        ...
        struct boss_lady_deathwhisperAI : public BossAI
        {
            ...
            {
                _Reset();
                _charmTargets[0] = NULL;
                _charmTargets[1] = NULL;
                _charmTargets[2] = NULL;
                        ...
                        case EVENT_DOMINATE_MIND_H:
                            ...
                            for(uint8 i = 0; i < _dominateMindCount; i++)
                                if(Unit* target = SelectTarget(SELECT_TARGET_RANDOM, 1, 0.0f, true, -SPELL_DOMINATE_MIND_H))
                                {
                                    ...
                                    _charmTargets[i] = target;
                                }
                            events.ScheduleEvent(EVENT_DOMINATE_MIND_H, urand(40000, 45000));
                            events.ScheduleEvent(EVENT_DOMINATE_MIND_ACTION, 2000);
                            break;
                        case EVENT_DOMINATE_MIND_ACTION:
                            if(!_charmTargets[0])
                                break;
                            if(_charmTargets[0]->HasAura(SPELL_DOMINATE_MIND_H))
                            {
                                for(uint8 j = 0; j < _dominateMindCount; j++)
                                {
                                    if(_charmTargets[j]->HasUnitState(UNIT_STAT_CASTING)) continue;
                                    Unit* pVictim = SelectTarget(SELECT_TARGET_RANDOM, 0, 30.0f, true, -SPELL_DOMINATE_MIND_H);
                                    switch(_charmTargets[j]->getClass())
                                    {
                                        case CLASS_PRIEST:
                                            if(_charmTargets[j]->ToPlayer()->HasTalent(47585, _charmTargets[j]->ToPlayer()->GetActiveSpec()))
                                            {
                                                if(!_charmTargets[j]->ToPlayer()->HasSpellCooldown(48127))
                                                    _charmTargets[j]->CastSpell(pVictim, 48127, false);
                                                else
                                                    _charmTargets[j]->CastSpell(pVictim, 48156, false);
                                                break;
                                            }
                                            Creature* cTarget;
                                            GetRandomSummonOrBoss(cTarget);
                                            _charmTargets[j]->CastSpell(cTarget, 48071, false);
                                            break;
                                        case CLASS_DRUID:
                                            if(_charmTargets[j]->ToPlayer()->HasTalent(50334, _charmTargets[j]->ToPlayer()->GetActiveSpec()))
                                            {
                                                if(_charmTargets[j]->HasAura(9634))
                                                    if(!_charmTargets[j]->ToPlayer()->HasSpellCooldown(48564))
                                                        _charmTargets[j]->CastSpell(_charmTargets[j]->getVictim(), 48564, false);
                                                break;
                                            }
                                            if(_charmTargets[j]->HasAura(33891))
                                            {
                                                Creature* cTarget;
                                                GetRandomSummonOrBoss(cTarget);
                                                _charmTargets[j]->CastSpell(cTarget, 50464, false);
                                                break;
                                            }
                                            if(_charmTargets[j]->HasAura(24858))
                                            {
                                                _charmTargets[j]->CastSpell(pVictim, 48461, false);
                                                break;
                                            }
                                            _charmTargets[j]->RemoveAurasDueToSpell(5487);
                                            _charmTargets[j]->RemoveAurasDueToSpell(9634);
                                            _charmTargets[j]->RemoveAurasDueToSpell(768);
                                            _charmTargets[j]->CastSpell(pVictim, 48461, false);
                                            break;
                                        case CLASS_HUNTER:
                                            _charmTargets[j]->CastSpell(_charmTargets[j]->getVictim(), 75, false);
                                            break;
                                        case CLASS_PALADIN:
                                            if(_charmTargets[j]->ToPlayer()->HasTalent(53563, _charmTargets[j]->ToPlayer()->GetActiveSpec()))
                                            {
                                                Creature* cTarget;
                                                GetRandomSummonOrBoss(cTarget);
                                                _charmTargets[j]->CastSpell(cTarget, 48785, false);
                                                break;
                                            }
                                            if(!_charmTargets[j]->ToPlayer()->HasSpellCooldown(20271))
                                                _charmTargets[j]->CastSpell(_charmTargets[j]->getVictim(), 20271, false);
                                            break;
                                        case CLASS_WARRIOR:
                                            if(_charmTargets[j]->ToPlayer()->HasTalent(46924, _charmTargets[j]->ToPlayer()->GetActiveSpec()))
                                            {
                                                if(!_charmTargets[j]->ToPlayer()->HasSpellCooldown(20271))
                                                    _charmTargets[j]->CastSpell(_charmTargets[j]->getVictim(), 47486, false);
                                                break;
                                            }
                                            if(_charmTargets[j]->ToPlayer()->HasTalent(46968, _charmTargets[j]->ToPlayer()->GetActiveSpec()))
                                            {
                                                if(!_charmTargets[j]->ToPlayer()->HasSpellCooldown(1680))
                                                    _charmTargets[j]->CastSpell(_charmTargets[j]->getVictim(), 1680, false);
                                                break;
                                            }
                                            if(_charmTargets[j]->ToPlayer()->HasTalent(46917, _charmTargets[j]->ToPlayer()->GetActiveSpec()))
                                            {
                                                if(!_charmTargets[j]->ToPlayer()->HasSpellCooldown(46968))
                                                    _charmTargets[j]->CastSpell(_charmTargets[j]->getVictim(), 46968, false);
                                                break;
                                            }
                                        case CLASS_WARLOCK:
                                            _charmTargets[j]->CastSpell(pVictim, 47809, false);
                                            break;
                                        case CLASS_MAGE:
                                            _charmTargets[j]->CastSpell(pVictim, 42842, false);
                                            break;
                                        case CLASS_ROGUE:
                                            _charmTargets[j]->CastSpell(_charmTargets[j]->getVictim(), 51723, false);
                                            break;
                                        case CLASS_DEATH_KNIGHT:
                                            _charmTargets[j]->CastSpell(_charmTargets[j]->getVictim(), 49895, false);
                                            break;
                                        case CLASS_SHAMAN:
                                            if(_charmTargets[j]->ToPlayer()->HasTalent(51490, _charmTargets[j]->ToPlayer()->GetActiveSpec()))
                                            {
                                                _charmTargets[j]->CastSpell(pVictim, 49238, false);
                                                break;
                                            }
                                            if(_charmTargets[j]->ToPlayer()->HasTalent(51533, _charmTargets[j]->ToPlayer()->GetActiveSpec()))
                                            {
                                                _charmTargets[j]->CastSpell(_charmTargets[j]->getVictim(), 17364, false);
                                                break;
                                            }
                                            if(_charmTargets[j]->ToPlayer()->HasTalent(61295, _charmTargets[j]->ToPlayer()->GetActiveSpec()))
                                            {
                                                Creature* cTarget;
                                                GetRandomSummonOrBoss(cTarget);
                                                _charmTargets[j]->CastSpell(cTarget, 55459, false);
                                                break;
                                            }
                                    }
                                }
                                events.ScheduleEvent(EVENT_DOMINATE_MIND_ACTION, urand(100, 1500));
                            }
                            break;
            ...
            void SpellHit(Unit* /*pCaster*/, SpellEntry const* spell)
            {
                if(me->GetCurrentSpell(CURRENT_GENERIC_SPELL))
                    if((me->GetCurrentSpell(CURRENT_GENERIC_SPELL)->m_spellInfo->Id == 71420) ||
                        (me->GetCurrentSpell(CURRENT_GENERIC_SPELL)->m_spellInfo->Id == 72007) ||
                        (me->GetCurrentSpell(CURRENT_GENERIC_SPELL)->m_spellInfo->Id == 72501) ||
                        (me->GetCurrentSpell(CURRENT_GENERIC_SPELL)->m_spellInfo->Id == 72502))
                        for(uint8 i = 0; i < 3; ++i)
            ...
            void GetRandomSummonOrBoss(Creature* &retSummon)
            {
                if(summons.empty())
                {
                    retSummon = me;
                ...
                retSummon = *cultistItr;
            }

        private:
            Unit* _charmTargets[3];

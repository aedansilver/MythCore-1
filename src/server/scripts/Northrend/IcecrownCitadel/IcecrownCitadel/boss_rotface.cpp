...
enum Events
{
    ...
    EVENT_STICKY_OOZE       = 7,
    EVENT_START_WALKING     = 8,
};

class InfectionTargetSelector
{
    public:
        InfectionTargetSelector(uint32 aura) : _aura(aura) { }
        bool operator()(Unit* unit) const
        {
            return unit->HasAura(_aura);
        }
    private:
        uint32 _aura;
};
    ...
    public:
        ...
        struct npc_big_oozeAI : public ScriptedAI
        {
            ...
            void IsSummonedBy(Unit* /*pSummoner*/)
            {
                ...
                events.ScheduleEvent(EVENT_STICKY_OOZE, 5000);
                events.ScheduleEvent(EVENT_START_WALKING, 2500);
    ...
    public:
        ...
        class spell_rotface_mutated_infection_SpellScript : public SpellScript
        {
            ...
            void FilterTargets(std::list<Unit*>& targets)
            {
                ...
                targets.remove_if(InfectionTargetSelector(GetSpellInfo()->Id));
                if(targets.empty())
                    return;

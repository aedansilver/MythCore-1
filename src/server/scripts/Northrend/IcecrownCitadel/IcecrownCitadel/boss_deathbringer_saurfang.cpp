...
enum Spells
{
    ...
    SPELL_MARK_OF_THE_FALLEN_CHAMPION_H = 72260,
    SPELL_MARK_OF_THE_FALLEN_CHAMPION_DMG = 72255,
...
class boss_deathbringer_saurfang : public CreatureScript
{
    public:
        ...
        struct boss_deathbringer_saurfangAI : public BossAI
        {
            ...
            {
                _Reset();
                me->SetReactState(_introDone ? REACT_DEFENSIVE : REACT_PASSIVE);
                if (!_introDone)
            ...
            void EnterCombat(Unit* who)
            {
                ...
                // Set new home position - needed after wipe
            ...
            void KilledUnit(Unit* victim)
            {
                if(victim->GetTypeId() == TYPEID_PLAYER)
                {
                    ...
                    if(victim->HasAura(SPELL_MARK_OF_THE_FALLEN_CHAMPION))
                        DoCast(me, SPELL_MARK_OF_THE_FALLEN_CHAMPION_H);
                }
            }
            ...
            void DamageDealt(Unit* /*pVictim*/, uint32& damage, DamageEffectType damageType)
            {
                ...
                // Cast Mark of the Fallen Champion damage trigger in case of DIRECT_DAMAGE
                Map* map = me->GetMap();
                    ...
                    for(Map::PlayerList::const_iterator itr = Players.begin(); itr != Players.end(); ++itr)
                    {
                        if(Player* player = itr->getSource())
                        {
                            ...
                            if(player->HasAura(SPELL_MARK_OF_THE_FALLEN_CHAMPION))
                                DoCast(player, SPELL_MARK_OF_THE_FALLEN_CHAMPION_DMG);
                        }
                    }
                }
            }
            ...
            void SpellHitTarget(Unit* target, SpellEntry const* spell)
            {
                switch(spell->Id)
                {
                    case SPELL_MARK_OF_THE_FALLEN_CHAMPION:
                        ...
                        break;
                    case SPELL_MARK_OF_THE_FALLEN_CHAMPION_DMG: // Mark of the Fallen Champion and hm, triggered id
                    case 72444:
                    case 72445:
                    case 72446:
                    ...
                    switch(id)
                    {
                        ...
                        case POINT_CORPSE:
                            if(Creature* deathbringer = ObjectAccessor::GetCreature(*me, _instance->GetData64(DATA_DEATHBRINGER_SAURFANG)))
                            {
                                ...
                                deathbringer->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NOT_SELECTABLE); //because s:70637 is not implemented
                                //TODO: here a packet should be sent to the players informing about mounted mob
                                //me->SendMonsterMoveTransport(deathbringer); doesn't work!
                            }
                    ...
                    switch(eventId)
                    {
                        ...
                        case EVENT_OUTRO_HORDE_4: // move
                            if(Creature* deathbringer = ObjectAccessor::GetCreature(*me, _instance->GetData64(DATA_DEATHBRINGER_SAURFANG)))
                            {
                                me->AddUnitMovementFlag(MOVEMENTFLAG_WALKING);
                                Position pos;
                                deathbringer->GetPosition(&pos);
                                me->GetMotionMaster()->MovePoint(POINT_CORPSE, pos);
                            }
                            break;
...
class spell_deathbringer_boiling_blood_periodic : public SpellScriptLoader
{
    public:
        spell_deathbringer_boiling_blood_periodic() : SpellScriptLoader("spell_deathbringer_boiling_blood_periodic") { }

        class spell_deathbringer_boiling_blood_periodic_AuraScript : public AuraScript
        {
            PrepareAuraScript(spell_deathbringer_boiling_blood_periodic_AuraScript);
            ...
            void OnPeriodic(AuraEffect const* /*aurEff*/)
            {
                ...
                if(pCaster->GetPower(POWER_ENERGY) != pCaster->GetMaxPower(POWER_ENERGY))
                    pOwner->CastCustomSpell(SPELL_BLOOD_LINK_DUMMY, SPELLVALUE_BASE_POINT0, 1, pCaster, true);
            }

            void Register()
            {
                OnEffectPeriodic += AuraEffectPeriodicFn(spell_deathbringer_boiling_blood_periodic_AuraScript::OnPeriodic, EFFECT_0, SPELL_AURA_PERIODIC_DAMAGE);
            }
        };

        AuraScript* GetAuraScript() const
        {
            return new spell_deathbringer_boiling_blood_periodic_AuraScript();
        }
};
...
class npc_blood_beast : public CreatureScript
{
    public:
        ...
        struct npc_blood_beastAI : public ScriptedAI
        {
            ...
            void DamageDealt(Unit* /*pVictim*/, uint32& /*damage*/, DamageEffectType /*damageType*/)
            {
                if(Unit* saurfang = me->GetCreature(*me, _instance->GetData64(DATA_DEATHBRINGER_SAURFANG)))
                    if(saurfang->GetPower(POWER_ENERGY) != saurfang->GetMaxPower(POWER_ENERGY))
                        me->CastCustomSpell(SPELL_BLOOD_LINK_DUMMY, SPELLVALUE_BASE_POINT0, 3, saurfang, true);
            }
...
void AddSC_boss_deathbringer_saurfang()
{
    ...
    new spell_deathbringer_boiling_blood_periodic;

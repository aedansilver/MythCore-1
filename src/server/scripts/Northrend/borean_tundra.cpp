    ...
    struct npc_counselor_talbotAI : public ScriptedAI
    {
        ...
        void UpdateAI(const uint32 diff)
        {
            ...
            if(me->GetAreaId() == 4125)
            {
                ...
                if(uiDeflectionTimer <= diff)
                {
                    DoCast(me->getVictim(), SPELL_DEFLECTION);
                    uiDeflectionTimer = urand(20000, 25000);
                } else uiDeflectionTimer -= diff;

                if(uiSoulBlastTimer <= diff)
                {
                    DoCast(me->getVictim(), SPELL_SOUL_BLAST);
                    uiSoulBlastTimer  = urand(12000, 18000);
                } else uiSoulBlastTimer -= diff;
            }
...
public:
    ...
    struct npc_hidden_cultistAI : public ScriptedAI
    {
        ...
        void UpdateAI(const uint32 diff)
        {
            if(uiEventTimer && uiEventTimer <= diff)
            {
                switch(uiEventPhase)
                {
                    case 1:
...
public:
    ...
    struct npc_caravanAI : public ScriptedAI
    {
        npc_caravanAI(Creature* pCreature) : ScriptedAI(pCreature)
        {
            Reset();
        }
        ...
        {
            return new npc_caravanAI (pCreature);
        }
        ...
        bool OnGossipHello(Player* player, Creature* creature)
        {
            ...
            if(player->GetQuestStatus(11593) == QUEST_STATUS_INCOMPLETE)
            {
                player->KilledMonsterCredit(25343,me->GetGUID());
                return true;
            }

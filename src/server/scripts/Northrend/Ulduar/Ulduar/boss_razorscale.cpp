...
{
    { 571.901f, -136.554f, GROUND_Z, 4.878f },
    { 589.450f, -134.888f, GROUND_Z, 4.567f },
    { 559.119f, -140.505f, GROUND_Z, 5.137f },
    { 606.229f, -136.721f, GROUND_Z, 4.340f },
};
                        ...
                        switch(eventId)
                        {
                            ...
                            case EVENT_SUMMON:
                                for(uint8 n = 0; n < RAID_MODE(2,4); ++n) {
                                    ...
                                    if(HarpoonCounter >= 1)
                                    {
                                        events.ScheduleEvent(EVENT_SUMMON, RAID_MODE(120000, 250000), 0, PHASE_FLIGHT);
                                        return;
                                    }
                                }
            ...
            void DoAction(int32 const action)
            {
                switch(action)
                {
                    case ACTION_EVENT_START:
                        ...
                        me->SetReactState(REACT_AGGRESSIVE);
                        Unit* pTarget = SelectTarget(SELECT_TARGET_RANDOM, 0, 800.0f, true);
                        if(pTarget)
                            pTarget->SetInCombatWith(me);
                        else
                            Reset();
                        DoZoneInCombat();
                        break;
    ...
    public:
        ...
        struct npc_darkrune_sentinelAI : public ScriptedAI
        {
            ...
            void UpdateAI(uint32 const diff)
            {
                ...
                if(ShoutTimer <= diff)
                {
                    DoCast(me, RAID_MODE(SPELL_BATTLE_SHOUT, SPELL_BATTLE_SHOUT_25));

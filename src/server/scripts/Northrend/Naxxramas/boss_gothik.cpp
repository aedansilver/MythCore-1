...
public:
    ...
    struct boss_gothikAI : public BossAI
    {
        ...
        {
            if(pSummoned->GetEntry() == WORLD_TRIGGER)
                pSummoned->setActive(true);
            else if(!mergedSides)
            {
                pSummoned->AI()->DoAction(me->HasReactState(REACT_PASSIVE) ? 1 : 0);
                pSummoned->AI()->EnterEvadeMode();
            } else {
                pSummoned->AI()->DoAction(0);

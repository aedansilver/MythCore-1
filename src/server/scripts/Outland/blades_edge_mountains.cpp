...
public:
    ...
    CreatureAI* GetAI(Creature* creature) const
    {
        return new npc_multi_spectrum_light_trap_bunnyAI(creature);
    }

    struct npc_multi_spectrum_light_trap_bunnyAI : public ScriptedAI
    {
        npc_multi_spectrum_light_trap_bunnyAI(Creature* c) : ScriptedAI(c)
        {
            GameObject* Trap = me->SummonGameObject(185011, me->GetPositionX(), me->GetPositionY(), me->GetPositionZ(), me->GetOrientation(), 0, 0, 0, 0, uint32(me->GetRespawnTime()));
                    ...
                    for(Map::PlayerList::const_iterator itr = players.begin(); itr != players.end(); ++itr)
                    {
                        if(Player* player = itr->getSource())
                        {
                            if(player->GetQuestStatus(10674) == QUEST_STATUS_INCOMPLETE)
                                player->KilledMonsterCredit(21929, 0);
...
public:
    ...
    {
        npc_evergrove_druidAI(Creature* c) : ScriptedAI(c)
        {

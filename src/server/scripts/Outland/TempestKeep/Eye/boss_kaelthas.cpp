...
struct advisorbase_ai : public ScriptedAI
{
    ...
    void Reset()
    {
        ...
        if(pInstance && (pInstance->GetData(DATA_KAELTHASEVENT) == 1 || pInstance->GetData(DATA_KAELTHASEVENT) == 3))
            if(Creature* Kaelthas = Unit::GetCreature((*me), pInstance->GetData64(DATA_KAELTHAS)))
    ...
    void DamageTaken(Unit* killer, uint32 &damage)
    {
        ...
        if(FakeDeath && pInstance && pInstance->GetData(DATA_KAELTHASEVENT) != 0)
        {
            damage = 0;
            return;
        }
    ...
    public:
        ...
        struct boss_kaelthasAI : public ScriptedAI
        {
            ...
            void StartEvent()
            {
                ...
                m_auiAdvisorGuid[0] = pInstance->GetData64(DATA_THALADREDTHEDARKENER);
                m_auiAdvisorGuid[1] = pInstance->GetData64(DATA_LORDSANGUINAR);
                m_auiAdvisorGuid[2] = pInstance->GetData64(DATA_GRANDASTROMANCERCAPERNIAN);
                m_auiAdvisorGuid[3] = pInstance->GetData64(DATA_MASTERENGINEERTELONICUS);
...
class boss_thaladred_the_darkener : public CreatureScript
{
    public:
        ...
        struct boss_thaladred_the_darkenerAI : public advisorbase_ai
        {
            ...
            void JustDied(Unit* /*pKiller*/)
            {
                if(pInstance && pInstance->GetData(DATA_KAELTHASEVENT) == 3)

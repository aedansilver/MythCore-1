        ...
        void UpdateAI(const uint32 diff)
        {
            if(!withbody)
            {
                ...
                if(laugh <= diff)
                {
                    ...
                    if(Creature* pCreature = DoSpawnCreature(HELPER, 0, 0, 0, 0, TEMPSUMMON_TIMED_DESPAWN, 1000))
                        pCreature->CastSpell(pCreature, SPELL_HEAD_SPEAKS, false);
...
public:
    ...
    struct mob_pulsing_pumpkinAI : public ScriptedAI
    {
        ...
        void JustDied(Unit* /*pKiller*/) { if(!sprouted) Despawn(); }
